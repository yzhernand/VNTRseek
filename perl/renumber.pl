...
if ($switch_sign) {
        foreach my $input (@files) {
                ...
                my $output_leb36_h = FileHandle->new(">$output_leb36") or die "Cannot open $output_leb36 for writing\n";
                my $input_leb36_h = FileHandle->new("<$input.leb36") or die "Cannot open $input.leb36 for reading\n";

		if ($use_index) {
			$output_index = "$input.index.tmp";
			$output_index_h = FileHandle->new(">$output_index") or die "Cannot open $output_index for writing\n";
			$input_index_h = FileHandle->new("<$input.index") or die "Cannot open $input.index for reading\n";
		}
			...
			if ($use_index) {
				...
				die "Numbers differ at line $UNR in $input.index and $input.leb36\n" if $leb36_line[0] != $index_line[0];
		...
		if ($use_index) {
			...
			$output_index_h->close or die "Cannot close $output_index file handle\n";
			$input_index_h->close or die "Cannot close $input.index file handle\n";
			move($output_index, "$input.index");
		}

                $output_leb36_h->close or die "Cannot close $output_leb36 file handle\n";
                $input_leb36_h->close or die "Cannot close $input.leb36 file handle\n";
