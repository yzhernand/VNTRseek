...
if ($argc<10) { die "Usage: insert_reads.pl  clusterfile indexfolder fastafolder rotatedfolder rotatedreffile strip_454_keytags dbname msdir tempdir ispairedreads\n"; }
...
print STDERR keys(%RHASH) . " positive entries inserted into hash. (plus $negcount neg reference ones not in hash) (". (time()-$timestart)  . ") secs\n\n";
...
#                or die "Couldn't prepare statement: " . $dbh->errstr;
$sth = $dbh->prepare("LOAD DATA LOCAL INFILE '$TEMPDIR/replnk_$DBNAME.txt' INTO TABLE replnk FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n';")
                or die "Couldn't prepare statement: " . $dbh->errstr;
...
foreach my $ifile (@indexfiles) {
 ...
 #   if ($3 =~  /\s(\d+)\s(\d+)\s(\d+\.\d)\s(\d+)\s([A-Z]+)/ ) {
     ...
     print $TEMPFILE $id,",",$HEADHASH{"$head"},",",$first,",",$last,",",$pat,",",$copy,",",$pattern,",",$profile,",",$profilerc,",",$proflen,"\n";
...
if ($inserted ==  keys(%RHASH)) {
  print STDERR "\n\n..." . $inserted  . " read repeat entries inserted into database (" . (time()-$timestart) . ") secs.\n\n";
} else {
 die "\n\nERROR: hash contains ".keys(%RHASH)." entries, while index files only have $processed matching entries and only $inserted were inserted into the database. Aborting!\n\n";
}
...
# get the fasta/fastq files
 ...
 $sth = $dbh->prepare("LOAD DATA LOCAL INFILE '$TEMPDIR/fastareads_$DBNAME.txt' INTO TABLE fasta_reads FIELDS TERMINATED BY '\t' LINES TERMINATED BY '\n';")
                or die "Couldn't prepare statement: " . $dbh->errstr;
				...
				$dnastr="";
				 ...
				 if ($_ =~ /^>(.*)(?:\n|\r)/) {
				  ...
				  if (exists $HEADHASH{"$headstr"}) {
				      ...
				      print $TEMPFILE $HEADHASH{"$headstr"},"\t","$headstr","\t","$dnastr","\n";
				...
				$qualstr="";
				 ...
				 if ($qmode==2 && $_ =~ /^\@(.*)(?:\n|\r)/) {
				  ...
				  #$qualstr = trimall($qualstr);
		    		  $qualstr = ""; # need to escape this field properly, currently causing problems
				  $totalReads++;
				  if (exists $HEADHASH{"$headstr"}) {
					...
					print $TEMPFILE $HEADHASH{"$headstr"},"\t","$headstr","\t","$dnastr","\t",substr($qualstr,4),"\n";

				      } else {

					print $TEMPFILE $HEADHASH{"$headstr"},"\t","$headstr","\t","$dnastr","\t",$qualstr,"\n";
...
if ($inserted ==  keys(%HEADHASH)) {
  print STDERR "\n\n..." . $inserted  . " reads inserted into database  (" . (time()-$timestart) . ") secs..\n\n";
} elsif ($inserted != $processed) {
 die "\n\nERROR: inserted into database ".$inserted." entries, while gzipped fasta files have $processed matching entries. Aborting!\n\n";
} else {
 die "\n\nERROR: hash contains ".keys(%HEADHASH)." entries, while gzipped fasta files only have $inserted matching entries. Aborting!\n\n";
}
