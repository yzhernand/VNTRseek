...
if ($argc<6) { die "Usage: pcr_dup.pl indexfolder profcleanfolder dbname msdir cpucount tempdir\n"; }
...
my $rrintersect = 0;
my $sth = $dbh->prepare("SELECT count(*) FROM rank INNER JOIN rankflank ON rank.refid=rankflank.refid AND rank.readid=rankflank.readid;")
...
my %PENTRIES = ();
 ...
 $sth = $dbh->prepare('CREATE TEMPORARY TABLE pduptemp (`refid` INT(11) NOT NULL, `readid` INT(11) NOT NULL, PRIMARY KEY (refid,readid)) ENGINE=INNODB;')
                or die "Couldn't prepare statement: " . $dbh->errstr;
...
foreach my $ifile (@indexfiles) {
 ...
 if ($ifile =~ /(\d+)\.seq\.pcr_dup/) {
   ...
   if (open(FILE, "$indexfolder/$ifile")) {
       ...
       my @keys = sort { $RCOUNTS{$a} <=> $RCOUNTS{$b} or $a <=> $b } keys %RCOUNTS;
       my $iditer = 1;
...
#$sth1->finish;
 ...
 close($TEMPFILE);
 $sth = $dbh->prepare("LOAD DATA LOCAL INFILE '$TEMPDIR/pduptemp_$DBNAME.txt' INTO TABLE pduptemp FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n';")
                or die "Couldn't prepare statement: " . $dbh->errstr;
 $sth->execute();
 $sth->finish;
 ...
 my $delfromtable = 0;
 my $query = "DELETE FROM t1 USING rank t1 INNER JOIN pduptemp t2 ON ( t1.refid = t2.refid AND t1.readid = t2.readid );";
 $sth = $dbh->prepare($query);
 $delfromtable=$sth->execute();
 $sth->finish;
 $query = "DELETE FROM t1 USING rankflank t1 INNER JOIN pduptemp t2 ON ( t1.refid = t2.refid AND t1.readid = t2.readid );";
 $sth = $dbh->prepare($query);
 $sth->execute();
 $sth->finish;
...
if (open(FILE, ">$pcleanfolder/result/$DBNAME.pcr_dup.txt")) {
 ...
 $rrintersect = 0;
 $sth = $dbh->prepare("SELECT count(*) FROM rank INNER JOIN rankflank ON rank.refid=rankflank.refid AND rank.readid=rankflank.readid;")
                or die "Couldn't prepare statement: " . $dbh->errstr;
 $sth->execute() or die "Cannot execute: " . $sth->errstr();
 $num = $sth->rows;
 if ($num) {
 ...
 $sth = $dbh->prepare("INSERT INTO pduptemp SELECT map.refid, map.readid FROM map INNER JOIN rank ON rank.refid=map.refid AND rank.readid=map.readid INNER JOIN rankflank ON rankflank.refid=map.refid AND rankflank.readid=map.readid WHERE rank.ties=0 OR rankflank.ties=0;"); # clear all pduptemp entries
 $sth->execute();
 $sth->finish;
 ...
 $query = "UPDATE pduptemp p, map pp SET bbb=1 WHERE pp.refid = p.refid AND pp.readid=p.readid;";
 $sth = $dbh->prepare($query);
 $i=$sth->execute();
 $sth->finish;
 ...
 if (open(FILE, ">$pcleanfolder/result/$DBNAME.ties.txt")) {
   ...
   $query = "SELECT map.refid, max(bbb) as mbb, (select head from fasta_ref_reps where rid=map.refid) as chr,(select firstindex from fasta_ref_reps where rid=map.refid) as tind  FROM map INNER JOIN rank ON rank.refid=map.refid AND rank.readid=map.readid INNER JOIN rankflank ON rankflank.refid=map.refid AND rankflank.readid=map.readid GROUP BY map.refid HAVING mbb=0 ORDER BY chr, tind;";
   $sth = $dbh->prepare($query);
   $sth->execute();
   $num = $sth->rows;
   $i=0;
   while ($i<$num) {
     my @data = $sth->fetchrow_array();
     $i++;
     print FILE "$i\t-".$data[0]."\t".$data[2]."\t".$data[3]."\n";     
   }
   $sth->finish;
   close (FILE);
 }
 ...
 if (open(FILE, ">$pcleanfolder/result/$DBNAME.ties_entries.txt")) {
   ...
   $query = "SELECT map.refid, map.readid,rank.ties,rankflank.ties  FROM map INNER JOIN rank ON rank.refid=map.refid AND rank.readid=map.readid INNER JOIN rankflank ON rankflank.refid=map.refid AND rankflank.readid=map.readid WHERE bbb=0 ORDER BY map.refid,map.readid;";
   $sth = $dbh->prepare($query);
   $sth->execute();
   $num = $sth->rows;
   $i=0;
   while ($i<$num) {
     my @data = $sth->fetchrow_array();
     $i++;
     print FILE "$i\t-".$data[0]."\t".$data[1]."\t".$data[2]."\t".$data[3]."\n";     
   }
   $sth->finish;
   close (FILE);
 }
...
$sth->finish;
 ...
 if ($delfromtable != $deleted) { die "Deleted number of entries($delfromtable) not equal to the number of deleted counter ($deleted), aborting! You might need to rerun from step 12."; }
...
sub fork_pcrdup {
        ...
        if ($pid == 0) {
		  ...
		  my $exstring = "./pcr_dup.exe $indexfolder/${_} $indexfolder/${_}.pcr_dup 0 2 > /dev/null";
		  system($exstring);
