...
my $HEADER_SUFFIX = ""; # set by program automatically for paired reads to distinguish headers
...
while (int($tarball_count/$files_to_process) > 200) { $files_to_process++; } 
...
FILE:
			...
			if (!close TRF) {
				if ($!)  {
				  print STDERR "Error closing trf process $trf_pid pipe: $!\n"; 
				  exit(1002);
				} elsif ($? != 0) { 
			...
			defined(my $trf2proclu_pid = open(TRF2PROCLU, "| $TRF2PROCLU_PARAM -o '$output_prefix.index' > '$output_prefix.leb36'"))
                                or die "Cannot start trf2proclu: $!\n";


                        # DEBUG
                        open(my $LOGFILE, ">${output_prefix}.log") or die "Can't open for writing logfile ('${output_prefix}.log')!";
                        $LOGFILE->autoflush;
                        ...
                        while (<GRANDCHILD>) {
                                ...
                                #print $LOGFILE $_;
                                $debug_trs_found++;
                                if (($debug_trs_found % 100) == 0) { print $LOGFILE "TRF output lines processed: $debug_trs_found\n"; }
                        ...
                        if (!close TRF2PROCLU)  {
				if ($!)  {
				  print STDERR "Error closing trf2proclu process $trf2proclu_pid pipe: $!\n"; 
				  exit(1002);
				} elsif ($? < -2) { 
				  print STDERR "trf2proclu process $trf2proclu_pid has returned $?!\n";
				  exit($?);
				}
			}
		...
		if ($core){
		     ...
		     print STDERR  "run_trf process $grandchild_pid has returned $rc!\n";
		     exit ($rc);
		}
