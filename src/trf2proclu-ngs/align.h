...
int ALPHABETSIZ =
  MAX_ALPHABETSIZ; /* the alphabet size for compositional alignment */
...
BASICALIGNPAIR *GetBasicPatternGlobalTextLocalAlignPair( char *sequence,
  char *pattern, int sequencelength, int patternlength, int alpha, int indel,
  int *submatrix );

WDPALIGNPAIR *GetWDPLocalAlignPair( char *sequence, char *pattern,
  int sequencelength, int patternlength, int alpha, int indel, int *submatrix );
...
CYCLICALIGNPAIR *GetCyclicDiffAlignPair(
  char *pat1, char *pat2, int len1, int len2 );

/* added by Gary Benson 1/2003 */
COMPOSITIONALIGNPAIR *GetCompositionGlobalAlignPair( char *seq1, char *seq2,
  int len1, int len2, int alpha, int indel, int *submatrix, int *M );

/* added by Gary Benson 1/2003 */
COMPOSITIONALIGNPAIR *GetCompositionLocalAlignPair( char *seq1, char *seq2,
  int len1, int len2, int alpha, int indel, int *submatrix, int *M );

/* added by Gary Benson 1/2003 */
int *GetCompositionSubstringMatchLengths( unsigned char *seq1,
  unsigned char *seq2, int len1, int len2, int limit, int ALPHABETSIZE );
...
#define min3switch( a, b, c ) \
    ( ( a <= b ) ? ( ( a <= c ) ? 1 : 3 ) : ( ( b <= c ) ? 2 : 3 ) )
#define max3switch( a, b, c ) \
    ( ( a >= b ) ? ( ( a >= c ) ? 1 : 3 ) : ( ( b >= c ) ? 2 : 3 ) )
#define max4switch( a, b, c, d )                                           \
    ( ( a >= b )                                                           \
        ? ( ( a >= c ) ? ( ( a >= d ) ? 1 : 4 ) : ( ( c >= d ) ? 3 : 4 ) ) \
        : ( ( b >= c ) ? ( ( b >= d ) ? 2 : 4 ) : ( ( c >= d ) ? 3 : 4 ) ) )
...
BASICALIGNPAIR *GetBasicPatternGlobalTextLocalAlignPair( char *seq1, char *seq2,
  int len1, int len2, int alpha, int indel, int *submatrix ) {
...
#ifdef _WIN_32_YES
    {
        ...
        /* this check was put here because of a problem of overflow when
        calculating the Slen and malloc() was just using the smaller number that
        came out as a result. A very hard and pesky error. Gelfand. */
        SlenCheck = (__int64) ( len1 + 1 ) * (__int64) ( len2 + 1 ) *
                    (__int64) ( sizeof( SD ) );
    ...
    Sp = S + ( len2 + 1 ) * bestscorerow +
         len2; /* set at bestscore element in final column */
...
/***********************************************************************/
WDPALIGNPAIR *GetWDPLocalAlignPair( char *sequence, char *pattern,
  int sequencelength, int patternlength, int alpha, int indel,
  int *submatrix ) {
    int           col, row;
...
#ifdef _WIN_32_YES
    {
        ...
        calculating the Slen and malloc() was just using the smaller number that
        came out as a result. A very hard and pesky error. Gelfand. */
        SlenCheck = (__int64) ( sequencelength + 1 ) *
                    (__int64) ( patternlength + 1 ) *
                    (__int64) ( sizeof( SD ) );
        if ( SlenCheck > (__int64) INT_MAX ) {
            return NULL;
        }
    ...
    for ( row = 1; row <= sequencelength; row++ ) {
        ...
        for ( col = 1; col <= patternlength; col++ ) {
            ...
            if ( col == 1 && ( Sp - 2 )->score > Srcm1p->score ) {
                sa = ( Sp - 2 )->score +
                     submatrix[26 * ( sequence[row - 1] - 'A' ) +
                               ( pattern[col - 1] - 'A' )];
                Sp->direction = WRAPDIAGONAL;
            } else {
                sa =
                  Srcm1p->score + submatrix[26 * ( sequence[row - 1] - 'A' ) +
                                            ( pattern[col - 1] - 'A' )];
                Sp->direction = DIAGONAL;
            }
    ...
    Sp = S + maxrow * ( patternlength + 1 ) +
         maxcol; /* set at maximum row and column */
    ...
    sequencepos =
      sequence + ( maxrow - 1 ); /* position at maximum score position */
...
*/
{
    ...
    size        = min( len1, len2 );
    Diffvectors = (COMPOSITIONVECTOR *) malloc(
      sizeof( COMPOSITIONVECTOR ) * ( size + 1 ) );
    if ( Diffvectors == NULL ) {
        return NULL; /* in case memory allocation fails */
    }
    ...
    for ( d = -m + 1; d <= n - 1; d++ ) {
        ...
        /* step through the substrings computing the difference X-Y at each
         * length */
        for ( g = 0; g < length; g++ ) {
            dfic++;
    ...
    in Radsort2 */
            ...
            for ( g = 0; g <= length; g++ ) {
                ...
                Radsort2[Counts[val + length] - 1] =
                  ( *rsi ); /* subtract 1 because Radsort is zero based */
                Counts[val + length]--;
                rsi--;
            }
        ...
        rsi      = Radsort1;
        previous = current = ( *rsi ); /* previous not used the first time
             through the loop */
        ...
        for ( g = 0; g <= length; g++ ) {
            ...
            if ( g == 0 ) {
                ...
                /* but only if the matching substring would be no longer than
                 * limit */
                if ( ( substring_length = current - previous ) > limit )
                    ...
                    match = 1;
                    for ( k = 0; k < ALPHABETSIZE; k++ ) {
                        if ( Diffvectors[current].C[k] !=
                             Diffvectors[previous].C[k] ) {
                            match = 0;
                            break;
                        }
                    }
                    ...
                    if ( match )
                        ...
                        substring_length = 0;
                    /* previous=current; */ /* uncomment for longest substring
                                               match */
                }
...
extra string (matchboundaries) which marks the boundaries of the shortest
composition matching substrings. M[i][j] is a matrix (len1+1)x(len2+1) which
holds the length of the shortest composition matching substrings (with zero<
length < limit) ending at seq1[i] and seq2[j] or zero if no such matching
substrings exist. The limit is the longest of these short substring that can be
included in the alignment. M is computed with the function
GetCompositionSubstringMatchLengths which includes the limit as a parameter.
This function must be passed M as a parameter. In the alignment, a substring
composition match is scored as the length of the substring times a matchscore
which is assumed to be a constant.  Here it is set to the score of A vs A.
*/
{
    int col, row, length;
    int eb;    /* equals e+indel */
    ...
    for ( row = 1; row <= len1; row++ ) {
        ...
        for ( col = 1; col <= len2; col++ ) {
            ...
            /* changed S computation, Gary Benson, 10/13/03 to allow comparison
            of mismatch and composition match greater than length 1 */
            ...
            if ( ( substringlen = *Mp ) > 1 ) {
                stemp = ( Sp - substringlen * ( len2 + 2 ) )->score +
                        substringlen * matchscore;
                if ( stemp > sa ) {
                    sa     = stemp;
                    satype = LONGDIAGONAL;
                }
...
COMPOSITIONALIGNPAIR *GetCompositionLocalAlignPair( char *seq1, char *seq2,
  int len1, int len2, int alpha, int indel, int *submatrix, int *M )
/*
Produces a local composition alignment for two sequences.  The aligned pair has
an extra string (matchboundaries) which marks the boundaries of the shortest
composition matching substrings. M[i][j] is a matrix (len1+1)x(len2+1) which
holds the length of the shortest composition matching substrings (with zero<
length < limit) ending at seq1[i] and seq2[j] or zero if no such matching
substrings exist. The limit is the longest of these short substring that can be
...
*/
{
    ...
    /* get the length of the alignment */
    Sp = S + bestscorerow * ( len2 + 1 ) +
         bestscorecol; /* set at bestscore element */
...
void CyclicAPToLimit(
  int start, int slength, int plength, CYCLICALIGNPAIR *pa ) {
...
CYCLICALIGNPAIR *GetBoundedDiffAlignment( char *pattern1, char *pattern2,
  int pattern1len, int start, int end, LIMIT *mins, LIMIT *maxs, SD *S ) {
    int              min, max;
    int              r, c, i, width;
    SD *             Sp, *Srm1p, *Scm1p, *Srcm1p; /* temporary pointers */
    int              sa, f, e;
    ...
    if ( max <
         MINIMUM( maxs[r + 1].right, end ) ) /* need to pad with VLN at end */
    {
    ...
    for ( r = 1; r <= pattern1len; r++ ) {
        ...
        if ( r < pattern1len &&
             max < MINIMUM( maxs[r + 1].right,
                     end ) ) /* need to pad with VLN at end */
        {
            for ( i = MINIMUM( maxs[r + 1].right, end ); i > max; i-- ) {
                Sp->score = VLN;
                Sp++;
    ...
    Sp                = S + ( pattern1len + 1 ) *
               (width) -1; /* position in last column of last row */
    ap->score = Sp->score;
...
CYCLICALIGNPAIR *GetCyclicDiffAlignPair(
  char *pattern1, char *pattern2, int length1, int length2 ) {
    ...
    if ( !gotreserve ) {
        ...
        for ( i = 0; i < LOGDEPTH; i++ ) {
            Reserve.limits[i] =
              (LIMIT *) malloc( sizeof( LIMIT ) * ( MAXPATSIZE + 1 ) );
    ...
    /* get first alignment */
    bestsofar = GetBoundedDiffAlignment( pattern1, doublepat, length1, 0,
      length2, L[0].limits, L[length2].limits, S );
    ...
    while ( ControlStack.height != -1 ) /* while stack is not empty */
    {
        ...
        if ( right - left > 1 ) {
            ...
            ap = GetBoundedDiffAlignment( pattern1, doublepat, length1, m,
              m + length2, L[left].limits, L[right].limits, S );
    ...
    if ( bestsofar->pattern2start == 1 )
        bestsofar->pattern2end = length2;
    else
        bestsofar->pattern2end = bestsofar->pattern2start - 1;
...
GAPPEDALIGNPAIR *GetGappedAlignPair( WDPALIGNPAIR *wdpap ) {
    ...
    if ( i > *cntpos )
        *cntpos = i; /* check for dashes at the end */

    /* calculate the total gapped length of alignment */
    for ( length = 0, cntpos = counts, patsrc = wdpap->patternside;
          *patsrc != '\0'; ) {
        if ( *patsrc != '-' ) {
            length += ( 1 + *cntpos );
            cntpos++;
    ...
    if ( gapap->patternside == NULL || gapap->sequenceside == NULL ) {
        free( gapap->patternside );
        free( gapap->sequenceside );
        free( gapap );
        return NULL;
    }
    ...
    /* fill in alignment */
    patsrc = wdpap->patternside;
    patdst = gapap->patternside;
    seqsrc = wdpap->sequenceside;
    seqdst = gapap->sequenceside;
    for ( i = 0, cntpos = counts; *patsrc != '\0';
          patdst++, patsrc++, seqdst++, seqsrc++ ) {
        if ( *patsrc == '-' )
            i++; /* i counts number of dashes actually found */
        else {
            for ( ; i < *cntpos; i++ ) {
                *patdst = *seqdst = '-';
                patdst++;
...
/*****************************************************************************************************/
void printBasicGlobalMatrix(
  char *seq1, char *seq2, int len1, int len2, SD *S ) {
...
/*****************************************************************************************************/
SD *GetBasicGlobalMatrix( char *seq1, char *seq2, int len1, int len2, int alpha,
  int indel, int *submatrix ) {
    int col, row;
    SD *S    = NULL;
    int Slen = 0;
    SD *Sp;
