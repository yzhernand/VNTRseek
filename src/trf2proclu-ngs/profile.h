...
void WriteProfileWithRC(
  FILE *fp, PROFILE *prof, PROFILE *profrc, char *left, char *right );
...
PAP *GetFixedProfileAPNarrowbandOld(
  PROFILE *p1, PROFILE *p2, unsigned char *sm, int homology ) {

    int col, row, length, lstart, rend, center, toplenreached, halfleft,
      oldrend;
    ...
    // double angle = acos(50.0 / ((200.0-homology-5)/2) ); /* to figure out the
    // middle radius to allow 60% similarity, 100 diagonal is 140 sum of 2 other
    // triangle sides */
    double angle =
      acos( ( ( homology - 5 ) / 2.0 ) /
            50.0 ); /* to figure out the middle radius to allow 60% similarity,
                       100 diagonal is 140 sum of 2 other triangle sides */
    ...
    for ( row = 1; row <= leftlen; row++ ) {
        ...
        lstart = center - (int) MAGIC_CENTER;
        lstart = max( 1, ( lstart ) );
        Srm1p  = Sp;
        Sp += ( toplen + 1 );
        Sp->direction = PA_DOWN;
...
PAP *GetFixedProfileAPNarrowband(
  PROFILE *p1, PROFILE *p2, unsigned char *sm, int homology ) {
    ...
    toplen  = p2->proflen;
    diagonal =
      sqrt( pow( toplen, 2 ) + pow( leftlen, 2 ) ); // actual diagonal lenght
    ...
    angle =
      acos( ( ( homology ) / 2.0 ) /
            50.0 ); // to figure out the middle radiusto allow 60% similarity,
                    // 100 diagonal is 140 sum of 2 other triangle sides
