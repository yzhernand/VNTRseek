...
// if this is set to 1, profiles are not rotated during sorting (must correspond to -i flag in redund.c)
...
// Warning:
// When modifying LBI_MAX_PATTERN_SIZE or LBI_MAX_SEQUENCE_SIZE, must also modify TRF_RECORD_FORMAT.
#define	LBI_MAX_PATTERN_SIZE	50000
#define	LBI_MAX_SEQUENCE_SIZE	520000
#define	TRF_RECORD_FORMAT	"%d %d %d %f %d %d %d %d %d %d %d %d %f %50000s %520000s %1000s %1000s"
...
int arsize_and_min_rep_sort( const void *item1, const void *item2 )
{
    ...
    */
    ...
    return pindcmp(p1->minRepresentation, p2->minRepresentation, p1->minrlen, p2->minrlen);
}
...
int main(int argc, char **argv)
{
    ...
    while (1) {
        ...
        c = getopt_long(argc, argv, "hf:m:s:i:p:l:o:", long_options, &option_index);
        ...
        case 'f':
            ...
            if (startid < 1)
                fputs("Starting id must be greater than or equal to 1. Aborting.\n", stderr);
    ...
    int nonopt = argc - optind;	// determine the number of non-option ARGV elements
    ...
    //	{
    //		sscanf(pattern,"%s %d %d %d %d %d %d",sequence,&match,&mismatch,&indel,&PM,&PI,&minscore);
    //	}
    //}
    ...
    while (!feof(fp)) {
        ...
        // scan TRF record(s)
        while (17 == fscanf(fp, TRF_RECORD_FORMAT,
                            &firstindex,
                            &lastindex,
                            &period,
                            &copynum,
            ...
            if (headerlen >= 6 && 0 == strcmp(repPtr->header + headerlen - 6, "_RCYES")) {
                ...
                firstindex = repPtr->firstindex; lastindex = repPtr->lastindex;

                repPtr->acount = tcount; repPtr->tcount = acount; repPtr->ccount = gcount; repPtr->gcount = ccount;
                acount = repPtr->acount; tcount = repPtr->tcount; ccount = repPtr->ccount; gcount = repPtr->gcount;
                free(repPtr->pattern); repPtr->pattern = GetReverseComplement(pattern);
                free(repPtr->sequence); repPtr->sequence = GetReverseComplement(sequence);

                strtemp = GetReverseComplement(repPtr->left);
                strtemp2 = GetReverseComplement(repPtr->right);
                free(repPtr->left); repPtr->left = strtemp2;
                free(repPtr->right); repPtr->right = strtemp;
            ...
            repPtr->copynum = (double)(rep.gappedlength) / (double)(rep.proflen);
            repPtr->conserved = rep.countmatch / (double)(rep.countmatch + rep.countmismatch + rep.countindel);
            ...
            /* skip periods less than minperiod or with flanks less than minflank */
            if (repPtr->patsize < minperiod || lenleft < minflank || lenright < minflank) {
                repPtr->spanning = 0;
            }
            ...
            if (1 /*repPtr->spanning*/) {
                ...
                repPtr->prof = profptr = (PROFILE *)smalloc( sizeof(PROFILE));

                profptr->key = repPtr->id;
                profptr->copynum = repPtr->copynum;
                profptr->patlen = repPtr->patsize;
                profptr->a = profptr->c = profptr->g = profptr->t = profptr->n = 0;
                ...
                for (i = 0; i < lentemp; i++) {
                    if (toupper(repPtr->sequence[i]) == 'A') profptr->a++;
                    else if (toupper(repPtr->sequence[i]) == 'C') profptr->c++;
                    else if (toupper(repPtr->sequence[i]) == 'G') profptr->g++;
                    else if (toupper(repPtr->sequence[i]) == 'T') profptr->t++;
                    else profptr->n++;
                }

                profptr->acgtCount = profptr->a + profptr->c + profptr->g + profptr->t;
            }
            ...
            if ( 1 /*repPtr->spanning*/) {
                ...
                repPtr->profrc = profptr = (PROFILE *)smalloc( sizeof(PROFILE));
                ...
                profptr->copynum = (double)(rep.gappedlength) / (double)(rep.proflen);
                profptr->proflen = rep.proflen;
                profptr->indices = smalloc( rep.proflen * sizeof(int));
                ...
                repPtr->minRepresentation = MinimumRepresentation(repPtr->prof->indices, repPtr->prof->proflen, repPtr->profrc->indices, repPtr->profrc->proflen, &(repPtr->minrlen), IDENTICAL_ONLY);
                ...
                if (!repPtr->spanning) {
                    ...
                    free(repPtr->prof); repPtr->prof = NULL;
                    free(repPtr->profrc); repPtr->profrc = NULL;
                }
    ...
    /* PIPELINE: arsize_and_min_rep_sort is now used to sort so that redundancy program can cleanup later */
    if (verbose)
        fputs("done\nsorting... ", stderr);
    ...
    if (verbose)
        fprintf(stderr, "done (%d items)\nwriting indexed .dat file... ", repList->size);
    ...
    /* this is a copy of index files with all results including nonspanning and all patsizes, needed for statistics at the end */
    ...
    for (tnode = repList->head; tnode != NULL; tnode = tnode->next) {
        ...
        /*fprintf(fp,"%d %d %d %d %.1f %d %d %d %d %d %d %d %d %.2f %s %s\n",repPtr->id,
        		repPtr->firstindex, repPtr->lastindex, repPtr->period,
        		repPtr->copynum, repPtr->patsize, repPtr->matchperc,
        ...
        if (repPtr->spanning)
        { fprintf(fp, "%d\t%s\t%d\t%d\t%.1f\t%d\t%s\n", repPtr->id, repPtr->header, repPtr->firstindex, repPtr->lastindex, repPtr->copynum, repPtr->patsize, repPtr->pattern); }
