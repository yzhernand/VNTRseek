...
int     ComputeBetterPattern(REPEAT *repeat,int alpha,int beta, int* sm)
{
    ...
    while(tryagain)
    {
        ...
        match=0; indel=0; mismatch=0;
        while(*ptr!='\0')
        {
            if(*ptr==*ptrpat) match++;
            else
            {
                if(*ptr!='-'&&*ptrpat!='-') mismatch++;
                else indel++;
            }
            ptr++;ptrpat++;
        }
        ...
        for(i=0,ptr=gap->sequenceside;*ptr!='\0';ptr++)
        {
            ...
            /* move index */
            i++;
            if(i==gap->gappedpatternlength) i=0;
        }
        ...
        repeat->profile = (COMPOSIT*) malloc(sizeof(COMPOSIT)*repeat->proflen);
		if (NULL == repeat->profile) return -1;
        ...
        {
            if(comps[i].consensus!='-') newlength++;
        }
