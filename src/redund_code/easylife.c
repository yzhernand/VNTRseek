 ...
 *  Version  :   1.04.
        - removed memory tracking stuff from unix version by introducing _WIN_32_YES flag,
	  the problem is actually not with unix but with 64 bit pointers, I have to rewrite it
          in later version
...
unsigned int _EL_GFSRstate[99]={40,
...
void EasyStringHashPrint( EASY_STRING_HASH* easyStringHash ) {
    ...
    for( i = 0; i < easyStringHash->size; i++ )
    {
        ...
        {
            printf("  %s (%d)", hitem->key, hitem->item ); /* prints the address of the item, not item itself */
        }
    }
...
void* simpleArrayCreateDebug( int ELEMENTS, int ESIZE, char *file, int line ) {
	...
	if ( NULL == array ) {
		doCriticalErrorAndQuit("\nsimpleArrayCreateDebug: Memory error allocating %d elements of size %d on line %d in file %s. Aborting!",ELEMENTS,ESIZE,line, file);
		exit(1);
	}
    ...
    if (EasyIntsHashGet( __EasyLifeSimpleArrayAddressHash, (int)array, -1 )!=NULL) {
    		doCriticalErrorAndQuit("\nsimpleArrayCreateDebug: Address %d already allocated. Line %d in file %s. You have a block somewhere in your code not delocated properly (make sure to use sfree for scalloc and saDestroy for SaCreate, etc. Aborting!", (int)array, line, file);
    		exit(1);
    } 
...
void simpleMemorySummaryDebug( int printUnfreed ) {
...
#ifdef _WIN_32_YES
            bytesLeft+=( ((__int64)(esadi->elements))*((__int64)(esadi->esize)) );
#else
    ...
    } else printf("Pass 1 to this function to view the unfreed allocations.\n"); 
}
