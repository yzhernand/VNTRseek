...
void init_complement_ascii(void) {
  /* complement has 256 entries so that finding the entries for A, C, G and T which are complement */
...
void  WriteProfileWithRC(FILE* fp, PROFILE* prof, PROFILE* profrc)
{
    ...
    if (prof->left&&prof->right) { fprintf(fp," %s|%s",prof->left,prof->right); }
