...
this is a restructured version of proclu, optimized for speed for refs vs reads
comparison, no cyclic alignment

  1.92 - passing 0 for maxerrors will now make the program pick one based on
length of the flank
  ...
  1.89 - added commmand line param to do ref-to-ref (only one flank has to
match)
...
void doEdgesBruteForce( FILE *fpi, FILE *fpi2, FILE *edgesIn,
  unsigned char *dt1, int fixed, char *outputfile );
...
/*******************************************************************************************/
CLUSTERBASE *doSearchSimilarities(
  FILE *fpi, FILE *fpi2, FILE *fpirot, FILE *fpi2rot, unsigned char *dt1 ) {
    ...
    int off1, i, j, pmin, TRANGE, NEWRANGE, lowerpat, higherpat, readcount,
      readpercent;
    size_t ui, uj;
    char * src, *sequence;
    ...
    for ( TRANGE = 1; TRANGE <= MAX_LIST_TRANGE; TRANGE++ ) {
        ...
        if ( TRANGE == 14 ) {
            Trange_Seed[TRANGE] =
              strdup( "1**11***1111**11*11**1*11*11*111**11**111*1*11" );
        } else if ( TRANGE == 13 ) {
            Trange_Seed[TRANGE] =
              strdup( "1**11***1111**11*11**1*11*11*111**11**111" );
        } else if ( TRANGE == 12 ) {
            Trange_Seed[TRANGE] =
              strdup( "1**11***1111**11*11**1*11*11*111**11" );
        } else if ( TRANGE == 11 ) {
            Trange_Seed[TRANGE] = strdup( "11*111*11111**11**111*11*11" );
        } else if ( TRANGE == 10 ) {
            Trange_Seed[TRANGE] = strdup( "11*111*11111**11**111*11*11" );
        } else if ( TRANGE == 9 ) {
            Trange_Seed[TRANGE] = strdup( "11*111*11111**11**111*11*11" );
            ...
            Trange_Seed[TRANGE] = strdup( "111*11111*111*111*11" );
        } else if ( TRANGE == 6 ) {
            Trange_Seed[TRANGE] = strdup( "111111*111111*11" );
        } else if ( TRANGE == 5 ) {
            Trange_Seed[TRANGE] = strdup( "111111*11111*11" );
        } // kb1 lost 1
        else if ( TRANGE == 4 ) {
            Trange_Seed[TRANGE] = strdup( "11111*1111" );
        } // kb1 lost 1
        else if ( TRANGE == 3 ) {
            Trange_Seed[TRANGE] = strdup( "11111*111" );
        }
        // else if (TRANGE==2) { Trange_Seed[TRANGE] = strdup("11111*11");  } //
        // kb1 lost 8
        else if ( TRANGE == 2 ) {
            Trange_Seed[TRANGE] = strdup( "1111*11" );
        } // added 1 one
        // else if (TRANGE==1) { Trange_Seed[TRANGE] = strdup("111*11");  } //
        // 111*1
        else if ( TRANGE == 1 ) {
            Trange_Seed[TRANGE] = strdup( "111*1" );
        } // because its too short sometimes
        else {
            doCriticalErrorAndQuit( "Unsupported TRANGE(%d)", TRANGE );
        };

        /* SEED+
             if (TRANGE==14){ Trange_Seed[TRANGE] =
        strdup("1**11***1111**11*11**1*11*11*111**11**111*1*11");  } else if
        (TRANGE==13){ Trange_Seed[TRANGE] =
        strdup("1**11***1111**11*11**1*11*11*111**11**111");  } else if
        (TRANGE==12){ Trange_Seed[TRANGE] =
        strdup("1**11***1111**11*11**1*11*11*111**11");  } else if (TRANGE==11){
        Trange_Seed[TRANGE] = strdup("11*111*11111**11**111*11*11");  } else if
        (TRANGE==10){ Trange_Seed[TRANGE] =
        strdup("11*111*11111**11**111*11*11");  } else if (TRANGE==9) {
        Trange_Seed[TRANGE] = strdup("111*11111*111*111*11");  }

        else if (TRANGE==8) { Trange_Seed[TRANGE] =
        strdup("111*11111*111*111*11");  } else if (TRANGE==7) {
        Trange_Seed[TRANGE] = strdup("111111*111111*11");  } else if (TRANGE==6)
        { Trange_Seed[TRANGE] = strdup("111111*11111*11");  } else if
        (TRANGE==5) { Trange_Seed[TRANGE] = strdup("11111*1111");  } else if
        (TRANGE==4) { Trange_Seed[TRANGE] = strdup("11111*111");  } else if
        (TRANGE==3) { Trange_Seed[TRANGE] = strdup("11111*11");  } else if
        (TRANGE==2) { Trange_Seed[TRANGE] = strdup("111*11");  } else if
        (TRANGE==1) { Trange_Seed[TRANGE] = strdup("111*1");  } // 111*1 else {
        doCriticalErrorAndQuit("Unsupported TRANGE(%d)",TRANGE); };
        */
        ...
        (TRANGE==10){ Trange_Seed[TRANGE] =
        strdup("11*111*11111**11**111*11*11");  } else if (TRANGE==9) {
        Trange_Seed[TRANGE] = strdup("111*11111*111*111*11");  } else if
        (TRANGE==8) { Trange_Seed[TRANGE] = strdup("111111*111111*11");  } else
        if (TRANGE==7) { Trange_Seed[TRANGE] = strdup("111111*11111*11");  }
        ...
        */
        ...
        for ( src = Trange_Seed[TRANGE]; *src != '\0'; src++ ) {
            if ( *src == '1' ) {
                Trange_Ones[TRANGE]++;
            }
        }
        ...
        if ( TRANGE <= MAX_ARRAY_TRANGE ) {
            ...
            Easy_Array_Tuples[TRANGE] = (EASY_ARRAY **) scalloc(
              Trange_N_Codes[TRANGE], sizeof( EASY_ARRAY ** ) );
    ...
    for ( pmin = 0; pmin <= MAXPROFILESIZE; pmin++ ) {
        ...
        if ( pmin >= 1111 ) {
            Trange_Profile_Range[pmin] = 14;
        } else if ( pmin >= 715 ) {
            Trange_Profile_Range[pmin] = 13;
        } else if ( pmin >= 474 ) {
            Trange_Profile_Range[pmin] = 12;
        } else if ( pmin >= 303 ) {
            Trange_Profile_Range[pmin] = 11;
        } else if ( pmin >= 201 ) {
            Trange_Profile_Range[pmin] = 10;
        }

        else if ( pmin >= 134 ) {
            Trange_Profile_Range[pmin] = 9;
        } else if ( pmin >= 89 ) {
            Trange_Profile_Range[pmin] = 8;
        } else if ( pmin >= 59 ) {
            Trange_Profile_Range[pmin] = 7;
        } else if ( pmin >= 39 ) {
            Trange_Profile_Range[pmin] = 6;
        } else if ( pmin >= 30 ) {
            Trange_Profile_Range[pmin] = 5;
        } else if ( pmin >= 25 ) {
            Trange_Profile_Range[pmin] = 4;
        } else if ( pmin >= 17 ) {
            Trange_Profile_Range[pmin] = 3;
        } else if ( pmin >= 10 ) {
            Trange_Profile_Range[pmin] = 2;
        } else if ( pmin >= 6 ) {
            Trange_Profile_Range[pmin] = 1;
        } else
            Trange_Profile_Range[pmin] = 0;
    }
    ...
    fprintf( stderr, "(total time: %.1lf secs)",
      (double) ( time( NULL ) - startTime ) );
    fprintf( stderr, "\nLoaded %zu profiles from reference file!",
      profileList->size / 2 );
    fflush( stderr );

    // read read profiles into list
    fprintf( stderr, "\nReading read profiles..." );
    fflush( stderr );
    off1 = 0;
    ...
    fprintf( stderr, "(total time: %.1lf secs)",
      (double) ( time( NULL ) - startTime ) );
    fprintf( stderr, "\nLoaded %zu profiles from reads file!",
      profileList2->size / 2 );
    fflush( stderr );
    ...
    fflush( stderr );
    {
        ...
        profileHash =
          CreateSingleHash( ( profileList->size + profileList2->size ) );
        ...
        for ( nof1 = profileList->head; nof1 != NULL; ) {
            ...
            profpair->prof->rotlist = profpair->profrc->rotlist =
              EasyListCreate( NULL, NULL );
            EasyListInsertTail( profpair->prof->rotlist, profpair );

            if ( NULL !=
                 GetSingleHashItem( profileHash, profpair->prof->key ) ) {
    ...
    // redundcy files
    if ( fpirot ) {
        fprintf( stderr, "\nUsing redundancy file to speed up alignments..." );
        fflush( stderr );

        if ( 0 != LoadRotated( fpirot ) )
            doCriticalErrorAndQuit( "Error reading rotated refs file!" );
    ...
    if ( fpi2rot ) {
        ...
        if ( 0 != LoadRotated( fpi2rot ) )
            doCriticalErrorAndQuit( "Error reading rotated reads file!" );
    ...
    // seed references
    fprintf( stderr, "\nSeeding reference profiles' concensus sequences..." );
    fflush( stderr );
    ...
    for ( nof1 = profileList->head; nof1 != NULL; ) {
        ...
        if ( pmin < 7 || pmin > MAXPROFILESIZE ) {
            doCriticalErrorAndQuit(
              "pmin(%d) must be in 7-%d range!", pmin, MAXPROFILESIZE );
        }
        ...
        if ( TRANGE > 0 ) {
            ...
            if ( seedstruct.hasX )
                doCriticalErrorAndQuit(
                  "Seeds with X are not allowed in this version. Aborting!" );

            sequence = seqdupPlusTuple( prof1->seq->length,
              prof1->seq->sequence, Trange_Tuple_Size[TRANGE] );
            blaststats.stTuplesProcessed += _HCLUST_process_sequence( sequence,
              &seedstruct, TRANGE, Trange_Tuple_Size[TRANGE], prof1, 0, pmin );
            sfree( sequence );

            sequence = seqdupPlusTuple( prof1rc->seq->length,
              prof1rc->seq->sequence, Trange_Tuple_Size[TRANGE] );
            blaststats.stTuplesProcessed +=
              _HCLUST_process_sequence( sequence, &seedstruct, TRANGE,
                Trange_Tuple_Size[TRANGE], prof1rc, 1, pmin );
            sfree( sequence );

            // lower range
            lowerpat = pmin - (int) ( pmin * PATLEN_SIZE_ERR_FRACTION +
                                      TRUNC_ROUND_CEIL );

            if ( lowerpat >= 7 &&
                 Trange_Profile_Range[lowerpat] <= ( TRANGE - 1 ) ) {
                ...
                if ( seedstruct.hasX )
                    doCriticalErrorAndQuit( "Seeds with X are not allowed in "
                                            "this version. Aborting!" );

                sequence = seqdupPlusTuple( prof1->seq->length,
                  prof1->seq->sequence, Trange_Tuple_Size[NEWRANGE] );
                blaststats.stTuplesProcessed +=
                  _HCLUST_process_sequence( sequence, &seedstruct, NEWRANGE,
                    Trange_Tuple_Size[NEWRANGE], prof1, 0, pmin );
                sfree( sequence );

                sequence = seqdupPlusTuple( prof1rc->seq->length,
                  prof1rc->seq->sequence, Trange_Tuple_Size[NEWRANGE] );
                blaststats.stTuplesProcessed +=
                  _HCLUST_process_sequence( sequence, &seedstruct, NEWRANGE,
                    Trange_Tuple_Size[NEWRANGE], prof1rc, 1, pmin );
                sfree( sequence );
            }

            // higher range
            higherpat = pmin + (int) ( pmin * PATLEN_SIZE_ERR_FRACTION +
                                       TRUNC_ROUND_CEIL );
    ...
    for ( TRANGE = 1; TRANGE <= MAX_LIST_TRANGE; TRANGE++ ) {
        ...
        if ( TRANGE <= MAX_ARRAY_TRANGE ) {
            ...
            for ( iter = Hash_Tuples[TRANGE]->walker->head; iter != NULL;
                  iter = iter->next ) {
    ...
    for ( TRANGE = 1; TRANGE <= MAX_LIST_TRANGE; TRANGE++ ) {
        ...
        if ( TRANGE <= MAX_ARRAY_TRANGE ) {
            ...
            Array_Tuples[TRANGE] = (SEED_HIT **) scalloc(
              Trange_N_Codes[TRANGE], sizeof( *( Array_Tuples[TRANGE] ) ) );
            Array_Tuples_Length[TRANGE] = scalloc( Trange_N_Codes[TRANGE],
              sizeof( *( Array_Tuples_Length[TRANGE] ) ) );
            Array_Tuples_Index[TRANGE]  = (EASY_ARRAY **) scalloc(
              Trange_N_Codes[TRANGE], sizeof( EASY_ARRAY ** ) );

            for ( ui = 0; ui < Trange_N_Codes[TRANGE]; ui++ ) {
                if ( ( artemp = Easy_Array_Tuples[TRANGE][ui] ) ) {
                    Array_Tuples[TRANGE][ui] =
                      ( shtemp = (SEED_HIT *) scalloc(
                          artemp->size, sizeof( SEED_HIT ) ) );
                    Array_Tuples_Index[TRANGE][ui] =
    ...
    for ( TRANGE = MAX_ARRAY_TRANGE + 1; TRANGE <= MAX_LIST_TRANGE; TRANGE++ ) {
        ...
        for ( iter = Hash_Tuples[TRANGE]->walker->head; iter != NULL;
            ...
            while ( gdrack ) {
                curr   = gdrack;
                gdrack = gdrack->next;
                SetDoubleHashItem( gdtemp, curr->key1, curr->key2, curr->data,
    ...
    for ( nof1 = profileList2->head; nof1 != NULL; ) {
        ...
        if ( TRANGE > 0 ) {
            ...
            // fprintf(stderr,"\nreadcount: %d => candidates1: %d candidates2:
            // %d (key: %d, pmin: %d, TRANGE: %d, read: %s, readrc:
            // %s)",readcount/2,candidates1->size,candidates2->size,prof1->key,pmin,TRANGE,prof1->seq->sequence,prof1rc->seq->sequence);
            // fflush(stderr);

            sequence = seqdupPlusTuple( prof1->seq->length,
              prof1->seq->sequence, Trange_Tuple_Size[TRANGE] );
            _HCLUST_find_candidates( sequence, prof1->seq, &seedstruct, TRANGE,
              Trange_Tuple_Size[TRANGE], 0, pmin, candidates1 );
            sfree( sequence );

            sequence = seqdupPlusTuple( prof1rc->seq->length,
              prof1rc->seq->sequence, Trange_Tuple_Size[TRANGE] );
            _HCLUST_find_candidates( sequence, prof1rc->seq, &seedstruct,
              TRANGE, Trange_Tuple_Size[TRANGE], 1, pmin, candidates2 );
            sfree( sequence );
        ...
        if ( (int) ( 100 * readcount / (double) profileList2->size ) >=
             readpercent ) {
            fprintf( stderr,
              "\n%d%% (ProfileAlignments: %llu, total time: %.1lf secs)",
              readpercent, blaststats.stProfileAlignments,
              (double) ( time( NULL ) - startTime ) );
            fflush( stderr );
            readpercent++;
        }
    ...
    fprintf( stderr,
      ...
      blaststats.stCandidatesConsidered, blaststats.stLCSalignments,
      blaststats.stProfileAlignments, blaststats.stProfileAlignmentsSuccess,
      blaststats.stPairsConfirmedAfterFlankAlignments, cb->trcount,
      (double) ( time( NULL ) - startTime ) );
...
int main( int argc, char **argv ) {
    ...
    char buffer[1000] = "", distfile1[1000] = "", outputfile[1000] = "",
         mapfile[1000] = "", edgesfile[1000] = "";
    ...
    if ( argc < 7 ) {
        ...
        printf(
          "\n\nUsage:  %s REFPROFILES READPROFILES DISTANCEFILE CUTOFF(70-100) "
          "MAXERRORS MAXFLANKCONSIDERED [EDGESINFILE] [OPTION]\n\n",
          argv[0] );
        printf( "\tOPTIONS: \n" );
        printf( "\t\t-r ref-to-ref alignments, only one flank has to match, 50 "
                "is default length but may be followed by integer flank "
                "length, ex -r 50 \n" );
    ...
    MAXFLANKCONSIDERED = min(
      1000000, atoi( argv[6] ) ); // Shortening of flanks for for ref-to-read.
                                  // Set this to anything and use -r [reflen]
                                  // for ref-to-ref flank shortening.

    // is option the next to last parameter?
    if ( argc >= 8 &&
         ( 0 == strcmp( argv[7], "-r" ) || 0 == strcmp( argv[7], "-R" ) ) ) {

        OPTION = 'R';

        if ( argc >= 9 && 0 != atoi( argv[8] ) ) {
            REFLEN = atoi( argv[8] );
        }
        ...
        if ( edgesIn &&
             0 != strcmp( edgesIn + strlen( edgesIn ) - 7, ".edgein" ) ) {
            printf(
    ...
    // -r
    if ( argc >= 9 &&
         ( 0 == strcmp( argv[8], "-r" ) || 0 == strcmp( argv[8], "-R" ) ) ) {
    ...
    if ( argc >= 10 &&
         ( 0 == strcmp( argv[9], "-r" ) || 0 == strcmp( argv[9], "-R" ) ) ) {

        OPTION = 'R';

        if ( argc >= 11 && 0 != atoi( argv[10] ) ) {
            REFLEN = atoi( argv[10] );
        }
    }
    ...
    // -m
    if ( argc >= 8 &&
         ( 0 == strcmp( argv[7], "-m" ) || 0 == strcmp( argv[7], "-M" ) ) ) {

        OPTION2 = 'M';
    }

    if ( argc >= 9 &&
         ( 0 == strcmp( argv[8], "-m" ) || 0 == strcmp( argv[8], "-M" ) ) ) {

        OPTION2 = 'M';
    }

    if ( argc >= 10 &&
         ( 0 == strcmp( argv[9], "-m" ) || 0 == strcmp( argv[9], "-M" ) ) ) {
    ...
    if ( MAXERRORS != 0 && OPTION == 'R' ) {
        fprintf( stderr, "\nERROR: maxerror must be 0 because it will be "
                         "dynamically generated based on flank length." );
        exit( 1 );
    }
    ...
    if ( edgesIn ) {
        ...
        if ( NULL == fpeIn ) {
            doCriticalErrorAndQuit(
              "Unable to open input file '%s'!", edgesIn );
        }
...
int LoadRotated( FILE *fp ) {
    ...
    if ( NULL == fp )
        return 0; /* no rotated file is fine */
    ...
    while ( 1 ) {
        ...
        while ( 1 ) {
            ...
            if ( i != 1 ) {
                doCriticalErrorAndQuit(
                  "Invalid format detected in the redundancy file!" );
            }
            ...
            if ( i != 1 || ( symbol != '\'' && symbol != '"' ) ) {
                doCriticalErrorAndQuit( "\n\nUnexpected value qualifier "
                                        "detected in the redundancy file!" );
            }
            ...
            if ( 1 == repeatcount ) {
                if ( NULL == ( profpair =
                                 GetSingleHashItem( profileHash, repeatkey ) ) )
                    doCriticalErrorAndQuit(
                      "\n\nError looking up key %d in profileHash!",
                      repeatkey );
    ...
    while ( fgets( buffer, 1000, edgesIn ) ) {
        if ( 2 == sscanf( buffer, "%d,%d", &id1, &id2 ) ) {
            ...
            bestsim = 0.0;
            {
                ...
                pap1 =
                  GetFixedProfileAP( profpair->prof, profpair2->prof, dt1 );
                pap2 =
                  GetFixedProfileAP( profpair->profrc, profpair2->prof, dt1 );
                ...
                pap1 =
                  GetFixedProfileAP( profpair->prof, profpair2->profrc, dt1 );
                pap2 =
                  GetFixedProfileAP( profpair->profrc, profpair2->profrc, dt1 );
            ...
            fprintf( fpo, "%d%c,%d,%.4lf\n", profpair->prof->key,
              ( dir == 0 ) ? '\'' : '\"', profpair2->prof->key, bestsim );
        }
    }
