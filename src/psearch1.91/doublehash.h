...
int     SetDoubleHashItem( GDHASH *hash, unsigned int key1, unsigned int key2,
      void *data, void *index, size_t length );
    ...
    *hkey = ( rjhash( key1 ) + rjhash( key2 ) ) % hash->size;
    ...
    if ( *hkey > hash->size ) {
        error_at_line( 1, 0, __FILE__, __LINE__,
          "bad hash key generated: %zu exceeds hash size %zu", *hkey,
          hash->size );
    }
