...
#define TRUNC_ROUND_CEIL	(1.0)	// truncate = 0.0, round = 0.5, ciel = 1.0 (default)

#define MAX_ARRAY_TRANGE 	(4)
#define MAX_LIST_TRANGE 	(14)
#define MAX_N_ONES 		(29)

EASY_ARRAY 	**Easy_Array_Tuples	[MAX_LIST_TRANGE + 1];	// range 1 - MAX_ARRAY_TRANGE
...
/************************************************************************************************************************/
int _HCLUST_process_sequence(char *Sequence, SEED_STRUCT *pSS, int TRANGE, int MaxTupleSize_SN, PROFILE* prof, int rc, int pmin ) {
  ...
  for(i=0;i<(Length);i++)
  {
    ...
    if((strchr("ACGT",Sequence[i])==NULL))  /* not one of A,C,G,T */
    {
      badcharindex=i;
      ...
      if(g<MaxTupleSize_SN) break;  /* i=Length and minimum tuple not found */
    }
    ...
    if ((i-badcharindex) >= MaxTupleSize_SN) /* check for valid tuple */
    {
    ...
    {
        {
          ...
          for(offset=0;offset<pSS->num_ones;offset++)
          {
			...
			//if (0 == rc)
                        //	where = i + pSS->roffset[0][pSS->num_ones - offset -1];
			//else
				where =  i + pSS->foffset[0][offset] ;

//fprintf(stderr,"\nOFFSET: %d, value: %d, i: %d, where: %d\n\n",offset,pSS->foffset[0][offset],i,where); 

                        if (where<0) { doCriticalErrorAndQuit("WHERE is <0!!"); }
                        if (where>=(Length+MaxTupleSize_SN)) { doCriticalErrorAndQuit("where too large!"); }
                        if (!(Sequence[where]=='A' || Sequence[where]=='C' || Sequence[where]=='G' || Sequence[where]=='T')) { doCriticalErrorAndQuit("Unknown character in sequence! (%c) seqlen: %d, trange: %d, MaxTupleSize_SN: %d, where: %d, rc: %d, sequence: %s\n",Sequence[where],Length,TRANGE,MaxTupleSize_SN,where,rc,Sequence); }

                        if (offset<=14)
                                seedcode=seedcode*4+Index[Sequence[ where ]];
                        else if (offset<=29)
                                seedcode2=seedcode2*4+Index[Sequence[ where ]];
                        else doCriticalErrorAndQuit("Seed too large! Aborting!");
          }
        }
        ...
        //Seedcode2[numpatterns]=seedcode2;
          ...
          /* create an entry for seed hit, use -location to indicated reverse complemented hit, distinction between shifted and unshifted is not needed */
          sh = (SEED_HIT*)scalloc(1, sizeof(SEED_HIT));
          sh->pmin = pmin;
          sh->dir = rc;
          sh->prof = prof;

	  if (pSS->num_ones <= 13 && TRANGE <= MAX_ARRAY_TRANGE && TRANGE>=1) {

		if (NULL == (artemp=Easy_Array_Tuples[TRANGE][seedcode])) {
		  Easy_Array_Tuples[TRANGE][seedcode] = artemp = EasyArrayCreate(2,NULL,free);
		}
		
		EasyArrayInsert(  artemp , (void*) sh);

	  } else if (pSS->num_ones <= MAX_N_ONES && TRANGE > MAX_ARRAY_TRANGE && TRANGE <= MAX_LIST_TRANGE) {

                if (NULL==(artemp=(EASY_ARRAY*)GetDoubleHashItem(Hash_Tuples[TRANGE],seedcode,seedcode2))) {
                  artemp = EasyArrayCreate(2,NULL,free);
                  SetDoubleHashItem(Hash_Tuples[TRANGE],seedcode,seedcode2,(void*)artemp,NULL,0);
                } 

		EasyArrayInsert(  artemp , (void*) sh);

	  } else {
		doCriticalErrorAndQuit("Seed with more than %d 1s or TRANGE higher than 7, not yet implemented!",MAX_N_ONES);
	  }
	  ...
	  /*
          ...
          if (NULL==(rc=GetTripleHashItem(SHash,seedcode,seedcode2,seedcode3))) {
...
/************************************************************************************************************************/
int _HCLUST_find_candidates(char *Sequence, FASTASEQUENCE *pseq, SEED_STRUCT *pSS, int TRANGE, int MaxTupleSize_SN, int rc, int pmin, EASY_ARRAY *candidates ) {
  ...
  for(i=0;i<(Length);i++)
  {
    ...
    /* produce code */
    {
	  ...
	  if (pSS->num_ones <= 13 && TRANGE <= MAX_ARRAY_TRANGE && TRANGE>=1) {
		...
		if (NULL != (iatemp=Array_Tuples_Index[TRANGE][seedcode])) {

                    for (uj=0; uj < iatemp->size; uj++) {

                       temp_hit = (SEED_HIT*)(iatemp->array[uj]);

//fprintf(stderr,"\n\tGOT HERE!, TRANGE: %d, temp_hit->pmin: %u",TRANGE,temp_hit->pmin); 

                       if (!start_hit && temp_hit->pmin >= (pmin - (int)(pmin * PATLEN_SIZE_ERR_FRACTION + TRUNC_ROUND_CEIL)))           { start_hit = temp_hit; }
                       if (!end_hit && temp_hit->pmin > (pmin + (int)(pmin * PATLEN_SIZE_ERR_FRACTION + TRUNC_ROUND_CEIL)))             { end_hit = temp_hit; }
                       if (start_hit && end_hit) { break; }
                    }

//                    if (!start_hit) { start_hit = & Array_Tuples[TRANGE][seedcode][ 0 ]; }
                    if (!end_hit) { end_hit = & Array_Tuples[TRANGE][seedcode][ Array_Tuples_Length[TRANGE][seedcode] - 1 ]; }

//fprintf(stderr,"\nGOT HERE!!, start_hit: %llu, end_hit: %llu",start_hit,end_hit); exit(1);
                ...
                if (NULL!=(gdhit=(GDHITEM*)GetDoubleHash(Hash_Tuples[TRANGE],seedcode,seedcode2))) {
                    ...
                    if (!end_hit) { end_hit = & shtemp[ length - 1 ]; }
...
//		   end_hit = & shtemp[ length - 1 ]; 
                    ...
                    if (start_hit && end_hit && start_hit <= end_hit) {
                       for ( ; ; start_hit++) {                       
			  //printf("\nstart_hit: %llu end_hit: %llu length: %d",start_hit,end_hit,length);
                          EasyArrayInsert(  candidates , (void*) start_hit);    
                          if (start_hit == end_hit) break;
                       }
                    }
...
/************************************************************************************************************************/
int _HCLUST_align_candidates(PROFILE *readprof, int rc, int pmin, EASY_ARRAY *candidates,unsigned char *dt1,CLUSTERBASE *cb ) {
  ...
  for (uj=0; uj < candidates->size; uj++) {
      ...
      if (pshit->prof == refprof) { continue; }
      refprof = pshit->prof;
      ...
      concensusLCS = LCS_multiple_word(refprof->seq->sequence,readprof->seq->sequence,refprof->seq->length,readprof->seq->length);

      shortest = min( readprof->seq->length,refprof->seq->length);
      minlen = (int)( shortest * LCS_CUTOFF + .5); // + .5 in there would round and would filter more stuff out
      minlen =   min( minlen, (shortest - 1) );
      if (concensusLCS < minlen) { continue; }
#endif
      ...
      if ( min( refprof->patlen, readprof->patlen ) >= 30 ) {
        ...
        if(refprof->proflen > readprof->proflen) 
          { sizeerror = (refprof->proflen/(double)readprof->proflen-1.0)*100; }
        else 
          { sizeerror = (readprof->proflen/(double)refprof->proflen-1.0)*100; }

        // narrowband sometimes returns error when patterns are too different (100% diff) in size so use regular
        if(sizeerror > 100.0) {
          pap1  = GetFixedProfileAP(refprof, readprof, dt1);
        } else {
          pap1  = GetFixedProfileAPNarrowband(refprof, readprof, dt1, TARGET_HOMOLOGY_INT);
        }
     ...
     blaststats.stProfileAlignments++;
			    ...
			    /* DEBUG 
			    if (refprof->key == 175388148 || refprof->key == -175388148 || refprof->key == 176296442 || refprof->key == -176296442) {
			    if (readprof->key == 175388148 || readprof->key == -175388148 || readprof->key == 176296442 || readprof->key == -176296442) {
				printf("\nDEBUG %d%s vs %d%s,\tprofscore = %.2lf\n",refprof->key, refprof->rcflag ? "RC" : "", readprof->key,readprof->rcflag ? "RC" : "",bestsim);				
			    }}
			    */
...
for (nrotf1=refprof->rotlist->head; nrotf1!=NULL; nrotf1=nrotf1->next)
{
for (nrotf2=readprof->rotlist->head; nrotf2!=NULL; nrotf2=nrotf2->next)
{
                               //fprintf(fpo,"%d,%d,%lf\n",prof1->key, prof2->key, bestsim);
...
//printf("\nbestsim: %.2l, fref ->: %d <-: %d   read -> : %d <-: %d",bestsim,prof1ROT->key,prof1rcROT->key,prof2ROT->key,prof2rcROT->key);
//printf("\nREF: %s|%s RC: %s|%s",prof1ROT->left,prof1ROT->right,prof1rcROT->left,prof1rcROT->right);
//printf("\nREAD: %s|%s RC: %s|%s",prof2ROT->left,prof2ROT->right,prof2rcROT->left,prof2rcROT->right);
//printf("\n");
                   ...
                   if (1 == pshit->dir) newdir=!newdir;

                   // flip dir if redundant ref is reversed
                   if (profpair1->prof->dir) newdir=!newdir;

                   // flip dir if redundant read is reversed
                   if (profpair2->prof->dir) newdir=!newdir;
       ...
       if (OPTION != 'R')  {
 		   ...
 		   int MAXEL1,MAXER1,LARGESERRORALLOWED;
	           ...
	           if (0 != MAXERRORS) {
                   ...
                   /* reflen is almost always 60, readlen is <=50. If reflen is from ends of chromosome it could be shorter.Thats why readlen will be shortened to reflen */
                   lerr = Edit_Distance_multiple_word_NoEndPenaltySeq1(ref_ptr->left,read_ptr->left,ref_ptr->leftlen,(ref_ptr->leftlen>read_ptr->leftlen)?read_ptr->leftlen:ref_ptr->leftlen);
                   rerr = Edit_Distance_multiple_word_NoEndPenaltySeq1(ref_ptr->right,read_ptr->right, ref_ptr->rightlen,(ref_ptr->rightlen>read_ptr->rightlen)?read_ptr->rightlen:ref_ptr->rightlen);

                   //fprintf(stderr,"\nref: %d read: %d (lflank: %d, rflank: %d) ===> lerr: %d (MAXEL1=%d), rerr: %d (MAXER1: %d)\n\n",ref_ptr->key,read_ptr->key,read_ptr->leftlen,read_ptr->rightlen,lerr,MAXEL1,rerr,MAXER1); 
	...
	/* refs vs refs */
           ...
           /* Dec 5, 2014: Yozen asked to  make ref-ref scoring just like regular pipeline runs */
 		   ...
 		   if (!flanksPasses) {
                	   ...
                	   lerr = Edit_Distance_multiple_word_NoEndPenaltySeq1(read_ptr->left,ref_ptr->left,read_ptr->leftlen,ref_ptr->leftlen);
	                   rerr = Edit_Distance_multiple_word_NoEndPenaltySeq1(read_ptr->right,ref_ptr->right,read_ptr->rightlen,ref_ptr->rightlen);
                   ...
                   /* if flanks failed, try the other way (for palindromes). This is more liberal for pipeline, but not sure if this should be in final version 
 
                   TURNS OUT THIS IS NOT NEEDED, because palindromes would be find on reverse seeds too, very 
                   unlikely for this to not be found, even impossible for true palindromes
			    ...
			    if (refprof->key == 175388148 || refprof->key == -175388148 || refprof->key == 176296442 || refprof->key == -176296442) {
			    if (readprof->key == 175388148 || readprof->key == -175388148 || readprof->key == 176296442 || readprof->key == -176296442) {
				printf("\nDEBUG %d%s vs %d%s,\tprofscore = %.2lf lerr=%d, rerr=%d\n",refprof->key, refprof->rcflag ? "RC" : "", readprof->key,readprof->rcflag ? "RC" : "",bestsim,lerr,rerr);				
			    }}
			    */
...
//printf("\nAdding link!");
		    ...
		    //  write to map file (new after  pscearch1.9)
		     ...
		     if (OPTION == 'R') {
			    ...
			    /*  exclude itself */
			    if (abs(prof1ROT->key) == abs(prof2ROT->key)) {
...
char* seqdupPlusTuple(int length, char *in, int tsize) {
        ...
        if (tsize>length) doCriticalErrorAndQuit("Tuple size (%d) is larger than sequence(%d). Aborting!",tsize,length);
...

{
  ...
  //memory_stats_print("\n init_complement: requesting", 256 * sizeof(long int) );
  if(Complement==NULL){doCriticalErrorAndQuit("\nInit_complement: Out of memory!");exit(0);}
...

{
  ...
  if(Index==NULL){doCriticalErrorAndQuit("\nInit_index: Out of memory!");exit(0);}
...

{
  ...
  //memory_stats_print("\n init_complement_ascii: requesting", 256 * sizeof(long int) );
  if(Complementascii==NULL){doCriticalErrorAndQuit("\ninit_complement_ascii: Out of memory!");exit(0);}
...
void retrieve_seed_info(char *seed,SEED_STRUCT *pSS)
{
    ...
    pSS->num_ones = num;
    if (num>MAX_N_ONES) {
        doCriticalErrorAndQuit("\n\nSorry, number of ones in the seed cannot exceed %d.", MAX_N_ONES );
        exit(0);
    }
...
void free_seed_info(SEED_STRUCT *pSS) {
    ...
    for (i = 0; i <= 1; i++)
    {
        if (pSS->foffset[i]) sfree(pSS->foffset[i]);
                pSS->foffset[i] = NULL;

        if (pSS->roffset[i]) sfree(pSS->roffset[i]);
                pSS->roffset[i] = NULL;
    }
}
