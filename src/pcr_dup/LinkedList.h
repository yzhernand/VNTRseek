...
//It has an itemIndex field so it can mimic an array.  It is like a mutable array (size mutable)
//It supports allocation, inserting data, returning data, and stepping through the list one item at a time
//The data is not part of the linked list.  It is a separate data structure which is pointed to by each element of the linked list
//Look closely at the procedures and use (in ReadGroup) of the two functions for While Loop
//The initializer sets the whileLoopPointer to the first element in the list
//The NextItem retrieves the data and updates the whileLoopPointer to the next element in the list
