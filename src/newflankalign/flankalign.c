    ...
    if (hasdata)
        while (1) {
            ...
            while (1) {
                ...
                if ((hasdata = fgets(buffer, MAXLINESIZE, rd_fp)) == NULL
                        || buffer[0] == '@') break;
            }
            ...
            if (pID == 0) {              // child
                ...
                for ( nd1 = read_list->head; nd1 != NULL; nd1 = nd1->next ) {
                    ...
                    fprintf(fp, "\n%lld	'	%d	%d	1	1	1	", read_ptr->id, read_ptr->leftlen,
                            read_ptr->rightlen);
                    ...
                    for ( nd2 = windowstart; nd2 != NULL; nd2 = nd2->next ) {
                        ...
                        lerr1 = Edit_Distance_multiple_word_NoEndPenaltySeq1(ref_ptr->left,
                                read_ptr->left, refleft,
                                (refleft > read_ptr->leftlen) ? read_ptr->leftlen : refleft);
                        rerr1 = Edit_Distance_multiple_word_NoEndPenaltySeq1(ref_ptr->right,
                                read_ptr->right, refright,
                                (refright > read_ptr->rightlen) ? read_ptr->rightlen : refright);
                        ...
                        /* If ref is from ends of chromosome it could be shorter. Thats why readlen will be shortened to reflen */
                        lerr2 = Edit_Distance_multiple_word_NoEndPenaltySeq1(ref_ptr->left,
                                read_ptr->rightcp, refleft,
                                (refleft > read_ptr->rightlen) ? read_ptr->rightlen : refleft);
                        rerr2 = Edit_Distance_multiple_word_NoEndPenaltySeq1(ref_ptr->right,
                                read_ptr->leftcp, refright,
                                (refright > read_ptr->leftlen) ? read_ptr->leftlen : refright);
                        ...
                        if ( (lerr1 <= MAXEL1 && rerr1 <= MAXER1) || (lerr2 <= MAXEL2
                                && rerr2 <= MAXER2) ) {
