/****************************************************************************************************
   ...
   Usage: ./flankalign outdir xmldir inputfile MAXERRORS MAXTHREADS PATLEN_SIZE_ERR
...
int main(int argc, char *argv[]) {
   ...
   /* cmd arguments */
   if(argc<8)
      doCriticalErrorAndQuit("\n\nFlankAlign - Please use ./flankalign  outdir xmldir inputfile MAXERRORS MAX_FLANK_CONSIDERED MAXTHREADS PATLEN_SIZE_ERR\n\n");
   ...
   if (MAXTHREADS<1) {
      doCriticalErrorAndQuit("\n\nFlankAlign - maxthreads should be more than or equal to one!\n\n");
   }
    ...
    while (1) {
     ...
     if (rid < 0) {
       ...
       *dst=0;
       ...
       if (templ_flank_ptr->leftlen > MAX_FLANK_CONSIDERED) templ_flank_ptr->left[MAX_FLANK_CONSIDERED]='\0';
       if (templ_flank_ptr->rightlen > MAX_FLANK_CONSIDERED) templ_flank_ptr->right[MAX_FLANK_CONSIDERED]='\0';
       /* ENDO OF FLANKSHORT */
...
//if (processed>=10) exit(0);
    ...
    if (pID == 0)                // child
    {
      // Code only executed by child process
       ...
       if (!fp) 
	   doCriticalErrorAndQuit("\n\nFlankAlign - can't open output file. Aborting!\n\n");

       fprintf(fp,"@%d_%d	1	1	1	1	1",a1,a2);
...
/*****************************************************************************************************************/
	  ...
	  /* if 0 passed for maxerror, calculate based on flank length */
  	  if (0 != MAXERRORS) {
		MAXEL1 = MAXERRORS;
		MAXER1 = MAXERRORS;
		MAXEL2 = MAXERRORS;
	  ...
	  /* shorten reference from 1000 to slightly more than readlen */
  	  if (0 == MAXERRORS) {
		MAXEL1 =  min( 8, (int)(0.4 * read_ptr->leftlen + .01));
		MAXER1 =  min( 8, (int)(0.4 * read_ptr->rightlen + .01));
		LARGESERRORALLOWED = max(MAXEL1,MAXER1);
	  }
	  refleft = min(ref_ptr->leftlen,read_ptr->leftlen + LARGESERRORALLOWED + 2);
	  refright = min(ref_ptr->rightlen,read_ptr->rightlen + LARGESERRORALLOWED + 2);

          /* If ref is from ends of chromosome it could be shorter. Thats why readlen will be shortened to reflen */
          lerr1 = Edit_Distance_multiple_word_NoEndPenaltySeq1(ref_ptr->left,read_ptr->left, refleft,(refleft>read_ptr->leftlen)?read_ptr->leftlen:refleft);
          rerr1 = Edit_Distance_multiple_word_NoEndPenaltySeq1(ref_ptr->right,read_ptr->right, refright,(refright>read_ptr->rightlen)?read_ptr->rightlen:refright);

          if (lerr1<0 || rerr1<0)
            doCriticalErrorAndQuit("\n\nFlankAlign - wrong sizes for narrowband alignment, seq1 must be larger. Aborting!\n\n");
...
/*****************************************************************************************************************/
	  ...
	  /* shorten reference from 1000 to slightly more than readlen */
  	  if (0 == MAXERRORS) {
		MAXEL2 =  min( 8, (int)(0.4 * read_ptr->rightlen + .01));
		MAXER2 =  min( 8, (int)(0.4 * read_ptr->leftlen + .01));
		LARGESERRORALLOWED = max(MAXEL2,MAXER2);
	  }
	  refleft = min(ref_ptr->leftlen,read_ptr->rightlen + LARGESERRORALLOWED + 2);
	  refright = min(ref_ptr->rightlen,read_ptr->leftlen + LARGESERRORALLOWED + 2);

          /* If ref is from ends of chromosome it could be shorter. Thats why readlen will be shortened to reflen */
          lerr2 = Edit_Distance_multiple_word_NoEndPenaltySeq1(ref_ptr->left, read_ptr->rightcp, refleft, (refleft>read_ptr->rightlen)?read_ptr->rightlen:refleft);
          rerr2 = Edit_Distance_multiple_word_NoEndPenaltySeq1(ref_ptr->right, read_ptr->leftcp, refright, (refright>read_ptr->leftlen)?read_ptr->leftlen:refright);
          ...
          //printf("%d - %d - %s - %s - %d\n",ref_ptr->leftlen, read_ptr->leftlen,ref_ptr->left, read_ptr->left, lerr);
          //printf("%d - %d - %s - %s - %d\n",ref_ptr->rightlen, read_ptr->rightlen,ref_ptr->right, read_ptr->right, rerr);
...
/*****************************************************************************************************************/
	  ...
	  /* if passes criteria in EITHER direction */
          if ( (lerr1<=MAXEL1 && rerr1<=MAXER1) || (lerr2<=MAXEL2 && rerr2<=MAXER2) ) {
            ...
            if ( (lerr1<=MAXEL1 && rerr1<=MAXER1) && (lerr2<=MAXEL2 && rerr2<=MAXER2) ) {
              ...
              if (sumerr1 <= sumerr2) 
                {  lerr = lerr1; rerr = rerr1; } 
              else 
                {  lerr = lerr2; rerr = rerr2; } 
