...
*   Last Revision Date  :   August 7, 2006
 ...
 *  qsort and insertion sort (which I modified to fit my data structures) come
from "Mastering Algorithms with C" by Kyle Loudon, published by O'Reilly &
Associates. Code used is surrounded by special comments. That is a great book, I
highly recommend it.


SIDE EFFECTS, LIMITATIONS:

 *  This library was tested on a Windows platform only, but with minor
modifications it should work on any platform.
 ...
 *  Version  :   1.04.
        - removed memory tracking stuff from unix version by introducing
_WIN_32_YES flag, the problem is actually not with unix but with 64 bit
pointers, I have to rewrite it in later version
        - fixed bug in the debug verseion of simpleArrayCreateDebug
        - fized size of memory clearing zombie structure which wasn't right,
            not really a bug

 *  Version  :   1.03.
        - fixed bgets func for lines followed with newlines
...
/* state table for random generator */
unsigned int _EL_GFSRstate[99] = {40, 1387256442, 539505633, 7126687,
  2115653676, 480642437, 1403109719, 898019591, 1609472695, 742049136,
  964528840, 1774590149, 531014893, 1478060509, 224730595, 1413365137,
  1415397063, 370513614, 1981855272, 1672294721, 1559669404, 1992066581,
  440083042, 1552169384, 949029171, 1848294689, 1014369863, 1226252978,
  199445637, 552539314, 101995811, 1795618857, 1468200845, 403608434, 466262418,
  1783034892, 2125486341, 1437171068, 839437811, 685760609, 311739045,
  1876584692, 223544964, 667792106, 1829604735, 887026472, 688815796,
  1153871680, 1135467106, 1975710098, 1393037901, 330755675, 804762632,
  393596594, 1695657725, 50479950, 1039358666, 1885424316, 400881551, 142829986,
  187416368, 821029919, 1292641081, 415120294, 1104581275, 1258423968,
  304285054, 400491932, 2014625087, 1619263031, 750624285, 1996732699, 97476312,
  1250544934, 2145510054, 1510875684, 262891578, 616032534, 1316668730,
  1500747974, 2138561534, 809719156, 1605036043, 510086967, 317411066, 54278455,
  2052774305, 439191668, 1881943474, 1397167115, 2046084812, 644321591,
  328615697, 1004646018, 1110120728, 2007784487, 992677826, 1756605308,
  796797739};

double        _EL_GFSRnormal = 2147483647.0;
unsigned int *_EL_GFSRtable  = _EL_GFSRstate;

/* return "random" number */
double EL_GFSR( void ) {
    unsigned int *sp = _EL_GFSRstate;
    unsigned int  k, p = sp[0];
    ...
    sp[p] ^= sp[k];
    sp[0] = p;
    return ( (double) sp[p] / _EL_GFSRnormal );
}
...
/* --------------------------------------------------------------------- */
void bclose( BSTREAM *bp ) { free( bp ); }
...
char *bgets( char *buffer, int lineLength, BSTREAM *bp ) {
    ...
    /* exit conditions */
    if ( NULL == bp || NULL == bp->lastTBFDataPos )
        return NULL;
    ...
    while ( *src != 13 && *src != 10 && *src != '\0' && count <= lineLength ) {
        buffer[count] = *src;
        src++;
        count++;
    }
...
EASY_ARRAY *EasyArrayCreate( size_t initialSize,
  void *( *copy )( const void *data ), void ( *destroy )( void *item ) ) {
    ...
    newEasyArray->reserved =
      initialSize > 0 ? initialSize : EASY_ARRAY_INITIAL_SIZE;
    newEasyArray->array =
      (void **) calloc( newEasyArray->reserved + 1, sizeof( void * ) );
...
EASY_ARRAY *EasyArrayCopy( EASY_ARRAY *easyArray, int copyData ) {
    ...
    if ( i != newEasyArray->size ) { /* one extra for NULL */
        doCriticalErrorAndQuit(
          "\nEasyArrayCopy: number of elements does not match the size field "
          "of the incoming list. Aborting!" );
        exit( 1 );
    }
...
EASY_ARRAY *EasyArrayCreateFromList( EASY_LIST *easyList, int copyData ) {
    ...
    /* create array elements (the same number as items in the easy list passed
     * in ) */
    ...
    if ( copyData ) {
        ...
        /* check if the copy function is provided */
        if ( NULL == newEasyArray->copy ) {
            doCriticalErrorAndQuit( "\nEasyArrayCreateFromList: No copy "
                                    "function provided. Aborting!" );
            exit( 1 );
        }

        /* create copies of items and place into the new array */
        for ( i = 0, eListIterator = easyList->head; eListIterator != NULL;
              eListIterator = eListIterator->next, i++ ) {
    ...
    if ( i != newEasyArray->size ) { /* one extra for NULL */
        doCriticalErrorAndQuit(
          "\nEasyArrayCreateFromList: number of elements does not match the "
          "size field of the incoming list. Aborting!" );
        exit( 1 );
    }
...
void EasyArrayInsert( EASY_ARRAY *easyArray, void *item ) {
    ...
    if ( arraySize >= easyArray->reserved ) {
        ...
        easyArray->array = (void **) realloc(
          easyArray->array, ( newReserved + 1 ) * sizeof( void * ) );
...
void *EasyArrayItem( EASY_ARRAY *easyArray, int position ) {
    ...
    if ( position < 0 || position >= easyArray->reserved ||
         NULL == ( array = (void **) easyArray->array[position] ) ) {
        doCriticalErrorAndQuit(
          "\nGetItemFromEasyArray: Illegal lookup OR Value Not Set (reserved "
          "range:0-%d, asked for %d. Aborting!",
          easyArray->reserved - 1, position );
        exit( 1 );
    }
...
void EasyArrayRemove( EASY_ARRAY *easyArray, int position ) {
    ...
    /* check if not NULL (note, empty means at least one element exists) */
    if ( NULL == easyArray || NULL == ( array = easyArray->array ) ||
         position < 0 || position >= easyArray->size ||
         NULL == array[position] ) {
        doCriticalErrorAndQuit(
...
void EasyListAppend(
  EASY_LIST *easyListTo, EASY_LIST *easyListFrom, int copyData ) {
    ...
    /* check if both exist */
    if ( NULL == easyListTo || NULL == easyListFrom )
        return;
...
EASY_LIST *EasyListCopy( EASY_LIST *easyList, int copyData ) {
    ...
    if ( copyData ) {
        ...
        /* create copies of items and place into the new array */
        for ( eListIterator = easyList->tail; eListIterator != NULL;
              eListIterator = eListIterator->prev ) {
...
/*
 *  This insertion sort and qsort algortihms come from the "Mastering Algorithms
...
/* ---------------------------------------------------------------------- */
static int partition( EASY_ARRAY *easyArray, int i, int k,
  int ( *compare )( const void *item1, const void *item2 ) ) {
    ...
    r[0] = EASYLIFE_INT_RANDRANGE( i, k ); //(rand() % (k - i + 1)) + i;
    r[1] = EASYLIFE_INT_RANDRANGE( i, k ); //(rand() % (k - i + 1)) + i;
    r[2] = EASYLIFE_INT_RANDRANGE( i, k ); //(rand() % (k - i + 1)) + i;
...
/* ---------------------------------------------------------------------- */
static void EasyArrayQuickSortHelper( EASY_ARRAY *easyArray, int i, int k,
  int ( *compare )( const void *item1, const void *item2 ) ) {
    ...
    if ( isSorted ) {
        ...
        while ( left <= right ) {
            ...
            default:
                ...
                /* This will happen if the compare function returned an illegal
                 * value. */
                doCriticalErrorAndQuit(
...
void *EasyArrayComputeMin( EASY_ARRAY *easyArray, int isSorted,
  int ( *compare )( const void *item1, const void *item2 ) ) {
    ...
    /* Return first value if the array is sorted or only 1 value is in the
     * array. */
    if ( isSorted || 1 == size ) {
...
EASY_STRING_HASH *EasyStringHashCreate( size_t initialSize,
  void *( *copy )( const void *data ), void ( *destroy )( void *item ) ) {
    ...
    newEasyStringHash =
      (EASY_STRING_HASH *) calloc( 1, sizeof( EASY_STRING_HASH ) );
    ...
    /* allocate the rack */
    newEasyStringHash->rack =
      (SHITEM **) calloc( initialSize, sizeof( SHITEM * ) );
...
/* ---------------------------------------------------------------------- */
void EasyIntsHashSet(
  EASY_INTS_HASH *easyIntsHash, void *item, int key1, ... ) {
...
size_t EasyLifeSomePrimes[] = {
  ...
  560689, 672827, 807403, 968897, 1162687, 1395263, 1674319, 2009069, 2410897,
  2893081, 3471697, 4166047, 4999273, 5999129, 7198963, 8638807, 10366597,
...
size_t EasyLifeGetPrime( size_t number ) {
    ...
    if ( number < EasyLifeSomePrimes[0] )
        return EasyLifeSomePrimes[0];
...
#ifdef _WIN_32_YES
static __int64 __EasyLifeSimpleArrayAllocations = 0;
static __int64 __EasyLifeSimpleArrayBytes       = 0;
#else
...
void *simpleArrayCreateDebug( int ELEMENTS, int ESIZE, char *file, int line ) {
    ...
    if ( NULL == array ) {
        doCriticalErrorAndQuit(
          "\nsimpleArrayCreateDebug: Memory error allocating %d elements of "
          "size %d on line %d in file %s. Aborting!",
          ELEMENTS, ESIZE, line, file );
        exit( 1 );
    }
    ...
    if ( !__EasyLifeSimpleArrayAddressHash ) {
        ...
        __EasyLifeSimpleArrayAddressHash =
          EasyIntsHashCreate( 10000000, NULL, EsadiFree );
    }

    /* remember array size and esize */
    esadi = (__EL_SIMPLE_ARRAY_DEBUG_INFO *) malloc(
      sizeof( __EL_SIMPLE_ARRAY_DEBUG_INFO ) );
    if ( NULL == esadi ) {
        doCriticalErrorAndQuit( "\nsimpleArrayCreateDebug: Memory error on "
                                "line %d in file %s. Aborting!",
          line, file );
        exit( 1 );
    }
    ...
    if ( EasyIntsHashGet( __EasyLifeSimpleArrayAddressHash, (int) array, -1 ) !=
         NULL ) {
        doCriticalErrorAndQuit(
          "\nsimpleArrayCreateDebug: Address %d already allocated. Line %d in "
          "file %s. You have a block somewhere in your code not delocated "
          "properly (make sure to use sfree for scalloc and saDestroy for "
          "SaCreate, etc. Aborting!",
          (int) array, line, file );
        exit( 1 );
    }
    ...
    __EasyLifeSimpleArrayBytes += ( ( ELEMENTS ) * ( ESIZE ) );
...
char *simpleArrayAtDebug( void *inArray, int POS ) {
...
#ifndef _WIN_32_YES
    doCriticalErrorAndQuit( "Not implemented!" );
    return NULL;
    ...
    if ( NULL == __EasyLifeSimpleArrayAddressHash ||
         NULL == ( esadi = (__EL_SIMPLE_ARRAY_DEBUG_INFO *) EasyIntsHashGet(
                     __EasyLifeSimpleArrayAddressHash, (int) ARRAY, -1 ) ) ) {

        // if (__EasyLifeSimpleArrayAddressHash)
        //    EasyIntsHashPrint( __EasyLifeSimpleArrayAddressHash );

        doCriticalErrorAndQuit( "\nsimpleArrayAtDebug: Could not lookup debug "
                                "info for address %d! Aborting!",
          ARRAY );
        exit( 1 );
    }
    ...
    if ( POS >= elements || POS < 0 ) {
        doCriticalErrorAndQuit(
          ...
          ARRAY, ARRAY + elements * esize - 1, esize, elements, POS,
          ARRAY + POS * esize, ARRAY + ( POS + 1 ) * esize - 1 );
        exit( 1 );
    }
...
void simpleArrayDestroyDebug( void *ARRAY, char *file, int line ) {
    ...
    if ( NULL == __EasyLifeSimpleArrayAddressHash ||
        ...
        doCriticalErrorAndQuit(
          "\nsimpleArrayDestroyDebug: Address %d is not present in the address "
          "hash table! Line %d in file %s. Aborting!",
          ARRAY, line, file );
        exit( 1 );
    }
...
void simpleMemorySummaryDebug( int printUnfreed ) {
...
#ifdef _WIN_32_YES
    __int64 bytesLeft       = 0;
    __int64 allocationsLeft = 0;
#else
    long long bytesLeft       = 0;
    long long allocationsLeft = 0;
#endif
    ...
    for ( i = 0; i < __EasyLifeSimpleArrayAddressHash->size; i++ ) {
        ...
        for ( hitem = __EasyLifeSimpleArrayAddressHash->rack[i]; hitem != NULL;
              hitem = hitem->next ) {
            esadi = (__EL_SIMPLE_ARRAY_DEBUG_INFO *) hitem->item;
#ifdef _WIN_32_YES
            bytesLeft += ( ( (__int64) ( esadi->elements ) ) *
                           ( (__int64) ( esadi->esize ) ) );
#else
            bytesLeft += ( ( (long long) ( esadi->elements ) ) *
                           ( (long long) ( esadi->esize ) ) );
#endif
            allocationsLeft += 1;
        }
    }
    ...
    printf( "\n"
      ...
      __EasyLifeSimpleArrayAllocations, allocationsLeft,
      __EasyLifeSimpleArrayBytes, bytesLeft );
#else
    /* print */
    printf( "\n"
    ...
    if ( printUnfreed ) {
        ...
        for ( i = 0; i < __EasyLifeSimpleArrayAddressHash->size; i++ ) {
            ...
            for ( hitem = __EasyLifeSimpleArrayAddressHash->rack[i];
                  hitem != NULL; hitem = hitem->next ) {
                esadi = (__EL_SIMPLE_ARRAY_DEBUG_INFO *) hitem->item;
        ...
        printf( "Pass 1 to this function to view the unfreed allocations.\n" );
}
...
void simpleMemorySummaryRelease( void ) {
    ...
    // doCriticalErrorAndQuit("\nsimpleMemorySummaryRelease: This operation is
    // not supported in the release mode. Aborting!"); exit(1);
}
...
void *simpleMatrixCreateDebug( int ROWS, int COLS, int ESIZE ) {
    ...
    if ( !__EasyLifeSimpleArrayAddressHash ) {
        ...
        __EasyLifeSimpleArrayAddressHash =
          EasyIntsHashCreate( 50000, NULL, free );
    }

    /* remember address of the rows array */
    if ( NULL == ( esadi = (__EL_SIMPLE_ARRAY_DEBUG_INFO *) malloc(
                     sizeof( __EL_SIMPLE_ARRAY_DEBUG_INFO ) ) ) ) {
        doCriticalErrorAndQuit(
    ...
    for ( i = 0; i < ROWS; i++ ) {
        ...
        esadi->elements = COLS;
        esadi->esize    = ESIZE;
        EasyIntsHashSet( __EasyLifeSimpleArrayAddressHash, esadi,
          (int) ( matrix + i * COLS * ESIZE ), -1 );
...
void simpleMatrixDestroyDebug( void *MATRIX ) {
    ...
    if ( NULL == __EasyLifeSimpleArrayAddressHash ||
         NULL == ( esadi = (__EL_SIMPLE_ARRAY_DEBUG_INFO *) EasyIntsHashGet(
                     __EasyLifeSimpleArrayAddressHash, (int) MATRIX, -1 ) ) ) {

        doCriticalErrorAndQuit( "\nsimpleMatrixDestroyDebug: Address %d is not "
                                "present in the address hash table! Aborting!",
          MATRIX );
        exit( 1 );
    }
    ROWS = esadi->elements;
    ...
    for ( i = 1; i < ROWS; i++ ) {
        ...
        EasyIntsHashSet(
          __EasyLifeSimpleArrayAddressHash, NULL, (int) rows[i], -1 );
    }
#endif
