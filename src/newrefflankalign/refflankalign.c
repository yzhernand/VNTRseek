/****************************************************************************************************
   ...
   Usage: ./refflankalign outdir xmldir inputfile MAXERRORS MAXTHREADS PATLEN_SIZE_ERR REFLEN
...
char *Complementascii = NULL;            /* precomputed array used for the reverse complement function */
int  ThreadCounter = 0;
...
int RC = 0; // this will be set by MinimumRepresentation if the reverse complement is the minimum
...
void MinimumRepresentation(char **indptr) {
 ...
 for (i=1; i<len; i++) {
   ...
   if (strcmp(src,tar)<0) { free(tar); tar = strdup(src);}
 ...
 for (i=1; i<len; i++) {
   ...
   if (strcmp(srcrc,tar)<0) { free(tar); tar = strdup(srcrc); RC=1; }
 }
...
int main(int argc, char *argv[]) {
    ...
    char		*outdir, *xmldir, *filename, *buffer, *src1, *src2, *src3, *src4, *src5, *dst, tempbuf[2000];
    FILE		*rd_fp,*RFP;
   ...
   /* cmd arguments */
   if(argc<8)
      doCriticalErrorAndQuit("\n\nRefFlankAlign - Please use ./Refflankalign  outdir xmldir inputfile MAXERRORS MAXTHREADS PATLEN_SIZE_ERR REFLEN\n\n");
   ...
   if (MAXTHREADS<1) {
      doCriticalErrorAndQuit("\n\nRefFlankAlign - maxthreads should be more than or equal to one!\n\n");
   }
   ...
   //   doCriticalErrorAndQuit("Memory error. Aborting!\n\n");
    ...
    if (Complementascii==NULL) {
        Complementascii = init_complement_ascii();
        if ( NULL == Complementascii ) return 1;
    }
   ...
   if (!RFP)
      doCriticalErrorAndQuit("\n\nRefFlankAlign - can't open output representative file (%s). Aborting!\n\n",tempbuf);
  ...
  while( (hasdata=fgets(buffer, MAXLINESIZE, rd_fp)) != NULL) {
    ...
    if (buffer[0]=='@') {
      ...
      if (a1<=0 || a2<=0) { printf("Can't read cluster number."); exit(1);       }
      break;
    }
  }; 
    ...
    while (1) {
     ...
     if (rid < 0) {
      ...
      //printf("rid: %ld\n",rid);
       ...
       src2 = src1;
       while (*src2!=',' && *src2!='\0') { *src2=toupper(*src2); if (NULL==strchr("ACGT",*src2)) { *src2='N'; } src2++; }
       if (*src2==',') { *src2='\0'; src2++; }
       ...
       src4 = src3;
       while (*src4!=',' && *src4!='\0') { *src4=toupper(*src4); if (NULL==strchr("ACGT",*src4)) { *src4='N'; } src4++; }
       if (*src4==',') { *src4='\0'; src4++; }
       ...
       templ_flank_ptr->leftlen = min(templ_flank_ptr->leftlen,REFLEN);
       templ_flank_ptr->rightlen = min(templ_flank_ptr->rightlen,REFLEN);
      
       templ_flank_ptr->leftcp =  GetComplement(templ_flank_ptr->left);
       templ_flank_ptr->rightcp =  GetComplement(templ_flank_ptr->right);
      ...
      //printf("rid: %ld\n",rid);
       ...
       if (!templ_flank_ptr->left || !templ_flank_ptr->right)
          doCriticalErrorAndQuit("\n\nRefFlankAlign - memory error 2. Aborting!\n\n");
     ...
     if ((hasdata=fgets(buffer, MAXLINESIZE, rd_fp)) == NULL || buffer[0]=='@') break;
    }
    ...
    if (pID == 0)                // child
    {
      // Code only executed by child process
       ...
       for ( nd1=ref_list->head; nd1!=NULL; nd1=nd1->next ) {
        ...
        //fprintf(fp,"%d - %d - %s - %s\n",read_ptr->leftlen, read_ptr->rightlen,read_ptr->left, read_ptr->right); fflush(fp);
        ...
        for ( nd2=nd1->next; nd2!=NULL; nd2=nd2->next ) { 
          ...
          //fprintf(fp,"\t%d - %d - %s - %s\n",ref_ptr->leftlen, ref_ptr->rightlen,ref_ptr->left, ref_ptr->right); fflush(fp);


	  /* if sizes are too different, stop */
          if(ref_ptr->patsize>read_ptr->patsize)
          {
                 sizeerror = (ref_ptr->patsize/(double)read_ptr->patsize-1.0)*100;
          }
          else
          {
                 sizeerror = (read_ptr->patsize/(double)ref_ptr->patsize-1.0)*100;
          }
	  ...
	  /* same direction */
  	  lerr1 = narrowbandUnitCostDistanceSameLengthBestScoreLastRowColumnLowMemUsesPointers(read_ptr->left,ref_ptr->left, read_ptr->leftlen,(ref_ptr->leftlen > read_ptr->leftlen)?ref_ptr->leftlen:read_ptr->leftlen,MAXERRORS);
 	  rerr1 = narrowbandUnitCostDistanceSameLengthBestScoreLastRowColumnLowMemUsesPointers(read_ptr->right,ref_ptr->right, read_ptr->rightlen,(ref_ptr->rightlen > read_ptr->rightlen)?ref_ptr->rightlen:read_ptr->rightlen,MAXERRORS);

          //lerr1 = rerr1 = 100;
          if (lerr1<0 || rerr1<0)
            { fprintf(fp,"\n\nRefFlankAlign - wrong sizes for narrowband alignment, seq2 must be larger. Aborting!\n\n"); _exit(1); }
 

          /* try aligning to the compliment of the other flank instead */
          lerr2 = narrowbandUnitCostDistanceSameLengthBestScoreLastRowColumnLowMemUsesPointers(read_ptr->rightcp,ref_ptr->left, read_ptr->rightlen,(ref_ptr->leftlen > read_ptr->rightlen)?ref_ptr->leftlen:read_ptr->rightlen,MAXERRORS);
          rerr2 = narrowbandUnitCostDistanceSameLengthBestScoreLastRowColumnLowMemUsesPointers(read_ptr->leftcp,ref_ptr->right, read_ptr->leftlen,(ref_ptr->rightlen > read_ptr->leftlen)?ref_ptr->rightlen:read_ptr->leftlen,MAXERRORS);
      ...
      for ( nd1=ref_list->head; nd1!=NULL; nd1=nd1->next ) {
        ...
        if (1 == ref_ptr->leftcon) leftcount++;
        if (1 == ref_ptr->rightcon) rightcount++;
        if (0 == ref_ptr->leftcon && 0 == ref_ptr->rightcon) distcount++;
        if (1 == ref_ptr->leftcon && 1 == ref_ptr->rightcon) undistcount++;
        if (1 == ref_ptr->leftcon || 1 == ref_ptr->rightcon) anycount++;
      }
 
      fprintf(fp,"    <class component_count=\"%d\" name=\"distinguishable\" ref_count=\"%d\" density=\"None\">\n",distcount?1:0,distcount);
if (distcount) {
      fprintf(fp,"      <nodes count=\"%d\">",distcount);
      for ( i=0,nd1=ref_list->head; nd1!=NULL; nd1=nd1->next ) {
	ref_ptr = (FLANK*)EasyListItem(nd1);
      ...
      fprintf(fp,"    <class name=\"left-connected\" ref_count=\"%d\" density=\"None\" component_count=\"%d\">\n",leftcount,leftcount?1:0);
if (leftcount) {
      ...
      fprintf(fp,"    <class name=\"right-connected\" ref_count=\"%d\" density=\"None\" component_count=\"%d\">\n",rightcount,rightcount?1:0);
if (rightcount) {
      fprintf(fp,"      <component density=\"None\">\n");
      ...
      fprintf(fp,"    <class name=\"left-or-right connected\" ref_count=\"%d\" density=\"None\" component_count=\"%d\">\n",anycount,anycount?1:0);
if (anycount) {
      fprintf(fp,"      <component density=\"None\">\n");
      ...
      fprintf(fp,"    <class name=\"undistinguishable\" ref_count=\"%d\" density=\"None\" component_count=\"%d\">\n",undistcount,undistcount?1:0);
if (undistcount) {
      fprintf(fp,"      <component density=\"None\">\n");
      ...
      totlensum = 0;
      largestPtr = ref_list->head ? (FLANK*)EasyListItem(ref_list->head) : NULL;
      for ( nd1=ref_list->head; nd1!=NULL; nd1=nd1->next ) {
        ...
        totlensum += ref_ptr->comparisons * (ref_ptr->leftlen + ref_ptr->rightlen);

        fprintf(fp,"    <reference>\n");   
        fprintf(fp,"      <pattern>%s</pattern>\n",ref_ptr->pattern);   
        fprintf(fp,"      <array>%s</array>\n",ref_ptr->sequence);   
        fprintf(fp,"    </reference>\n");   
      ...
      if (largestPtr)
      {
        ...
        if (!fprtemp)  
            { fprintf(fp,"\n\nRefFlankAlign - error opening temp representative file. Aborting!"); _exit(1); }
        ...
        fprintf(fprtemp,"%d_%d\t%s\t%s",a1,a2,scorebf,largestPtr->pattern);
        fclose(fprtemp);
      }
    ...
    {
	doCriticalErrorAndQuit("\n\nRefFlankAlign - failed to fork code=%d (%s). Aborting!\n\n",errno,strerror(errno));
        exit(1);
    } 
    ...
    /* if full batch scheled, wait for one to finishe and schedule another */
    ...
    a1=a2=0;
    sscanf(buffer,"@(%d_%d)",&a1,&a2);
    if (a1<=0 || a2<=0) { printf("Can't read cluster number. \n%s\n",buffer); exit(1);       }
  ...
  while( (hasdata=fgets(buffer, MAXLINESIZE, rd_fp)) != NULL) {
    ...
    if (buffer[0]=='@') {
       ...
       if (a1>0 && a2>0) { 
         ...
         if (!fprtemp)
            doCriticalErrorAndQuit("\n\nRefFlankAlign - error opening temp representative file (%s). Aborting!",tempbuf);
