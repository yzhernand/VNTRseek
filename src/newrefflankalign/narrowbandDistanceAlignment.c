...
int narrowbandUnitCostDistanceNoEndPenaltySeq2(char *seq1, char *seq2, int len1, int len2, int maxerr)
{
	...
	for (row=1; row<=len1; row++) 
	{
		...
		if (to>len2) to=len2;
		//add boundary condition
		if(from>0) S[row][from-1]=1000;
		//columns
		for (col=from; col<=to; col++) {
			if(col==0)S[row][0]=row;
			else 
			{
				...
				//if ((row==3)&&(col==3)) {
				//	printf("\nm:%d, d:%d, l:%d, u:%d S[%d][%d]:%d,seq1[%d]:%c,seq2[%d]:%c",
	...
	for (row=0; row<=len1; row++) {
		...
		to=row+maxerr;
		for (col=0; col<=len2; col++) {
			if((col<from-1)||(col>to+1))printf(" | ");
			else
			{
				if(S[row][col]==1000)printf(" X ");
				else printf("%2d ",S[row][col]);
			}
		}
	}
#endif
...
int narrowbandUnitCostDistanceNoEndPenaltySeq2LowMem(char *seq1, char *seq2, int len1, int len2, int maxerr)
{
	...
	for (row=0; row<=len1; row++) {
		...
		if (from<=0) from=0;
		else leftboundary=1; //true
		rightboundary=0; //false
		if (to>=len2) to=len2;
		else rightboundary=1; //true
		rowlength=to-from+1;
		S[row]=(int *)calloc(leftboundary+rowlength+rightboundary, sizeof(int));
	}
	...
	for (row=1; row<=len1; row++) 
	{
		...
		for (col=leftboundary; col<=leftboundary+rowlength-1; col++) {
			if(col==0)S[row][0]=row; //column zero fill
			else {				
				match=(seq1[row-1]==seq2[from+col-leftboundary-1])?0:1;
				up=S[row-1][col+previousleftboundary]+1;
				left=S[row][col-1]+1;
				diagonal=S[row-1][col-(1-previousleftboundary)]+match;
				S[row][col]=min3(diagonal,left,up);
			}
			//if ((row==1)&&(col==1)) {
			//	printf("\nrow:%d, col:%d,seqleft:%c, seqtop:%c\n",row,col,seq1[row-1],seq2[from+col-leftboundary-1]);
			//}
		}
		if (rightboundary) { //boundary condition right
			S[row][leftboundary+rowlength+rightboundary-1]=1000;
	...
	for (row=0; row<=len1; row++) {
		...
		for (col=0; col<from-1; col++) printf(" | ");
		for (col=0; col<=leftboundary+rowlength+rightboundary-1; col++) 
		{
			if(S[row][col]==1000)printf(" X ");
			else printf("%2d ",S[row][col]);
	...
	score=S[row][0];
	for (col=1;col<=leftboundary+rowlength+rightboundary-1; col++)
	{
		if (S[row][col]<score) {
			score=S[row][col];
		}
...
int narrowbandUnitCostDistanceNoEndPenaltySeq2LowMemUsesPointers(char *seq1, char *seq2, int len1, int len2, int maxerr)
{
	...
	for (row=1; row<=len1; row++) 
	{
		...
		Spd+=leftboundary;
		for (col=leftboundary; col<=leftboundary+rowlength-1; col++) {
			if(col==0)(*Sp)=row; //column zero fill
			else {				
				match=(seq1[row-1]==seq2[from+col-leftboundary-1])?0:1;
				up=(*Spu)+1;
				left=(*Spl)+1;
...
int narrowbandUnitCostDistanceSameLengthBestScoreLastRowColumnLowMemUsesPointers(char *seq1, char *seq2, int len1, int len2, int maxerr)
{
	//seq 1 is on left side
	//seq 2 is across top
