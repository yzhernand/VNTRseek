...
# Start position is just $2-(R+B) since bedtools start pos is 0-based
awk -F "," -v R=$READ_LENGTH -v F=$FLANK_LENGTH -v B=BUFFER '(NR>1){
	print $5 "\t" ($2-(R+B)) "\t" ($3+F+B) "\t" $1;
	print $5 "\t" ($2-(F+B)) "\t" $3+B "\t" $1;
}' "$SEQ_FILE" > "$TMPFILE"

bedtools getfasta -fi $REFERENCE -bed "$TMPFILE" | awk 'NR%2==0{ print}' | paste "$TMPFILE" - | \
awk -v R=$READ_LENGTH '{if(length($5)>=R){for (i=0; i<=length($5)-R; i++) {print ">" $1 ":" ($2+i) "-" ($2+i+R) "_" $4 "\n" substr($5,i,R)}} }'
