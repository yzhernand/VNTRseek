...
case  $runnext  in
  0|1|4|10|13|15) 				# multiple
    ...
    qsub -cwd -N vntrseek_"$dbsuffix"_step"$runnext" -l mem_total=64G -l h_rt=40:00:00 -pe omp $nprocs -v DBSUFFIX=$dbsuffix,NPROCS=$nprocs,WORKD=$WORKD,runnext=$runnext $scriptname
    ;;
  2|3|5|6|7|8|9|11|12|14|16|17|18|19)      	# single
    echo "runnext state ($runnext), running in SINGLE mode!"
    qsub -cwd -N vntrseek_"$dbsuffix"_step"$runnext" -l h_rt=40:00:00 -v DBSUFFIX=$dbsuffix,NPROCS=$nprocs,WORKD=$WORKD,runnext=$runnext $scriptname
    ;;
  20)
